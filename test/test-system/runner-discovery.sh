...
# Build a scratch suite holding one passing .sh probe and one bats file, the
# shared shape for the bats-propagation cases below.
create_bats_probe_suite() { # <suite-dir>
  local suite="$1"
  mkdir -p "$suite"
...
assert_fd3_drain_does_not_swallow_tests() {
  local runner_output runner_status suite="$work/fd3"
  mkdir -p "$suite"
...
assert_failing_find_fails_suite() {
  local runner_output runner_status suite="$work/pfind"
  mkdir -p "$suite/bin"
  write_probe_script "$suite/a.sh" 'exit 0'
...
assert_failing_sort_fails_suite() {
  local runner_output runner_status suite="$work/psort"
  mkdir -p "$suite/bin"
  write_probe_script "$suite/a.sh" 'exit 0'
...
assert_failing_bats_fails_suite() {
  local runner_output runner_status suite="$work/batsfail"
  create_bats_probe_suite "$suite"
  mkdir -p "$suite/failbin"
...
assert_passing_bats_leaves_suite_green() {
  local runner_output runner_status suite="$work/batspass"
  create_bats_probe_suite "$suite"
  mkdir -p "$suite/passbin"
