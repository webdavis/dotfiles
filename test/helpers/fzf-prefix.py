...
process = subprocess.Popen(['bash', '--noprofile', '--norc', '-i'], stdin=slave, stdout=slave, stderr=slave,
                           cwd=root, env={**os.environ, 'TERM': 'xterm', 'INPUTRC': '/dev/null', 'PS1': 'CHECK> '})
os.close(slave)
...
try:
    setup = ('source dot_bash_bindings_functions; source dot_bash_bindings; source dot_fzf_bindings; '
    ...
    assert b'\r\nREADY\r\nCHECK> ' in read_until(b'\r\nREADY\r\nCHECK> ', 0.4), 'Shell did not initialize'
