...
spec = importlib.util.spec_from_file_location('picker_git', root / 'dot_local/libexec/fzf-pickers/git_files.py')
assert spec and spec.loader and Path(spec.origin).is_file(), 'Git picker provider is missing'
...
with tempfile.TemporaryDirectory() as directory:
    ...
    assert {Path(r['path']).relative_to(p).as_posix() for r in rows} == {'with space', 'line\nbreak', '.hidden', 'sub/file'}
    ...
    historical = list(m.collect('historical-files', {**state, 'commit': git('rev-parse', 'HEAD').decode().strip()}))
