...
grep -qF "$log_route" "$TEMPLATE" ||
  fail "run_after_68 does not probe the '$log_route' route the log library posts to"
grep -qF 'relay' "$TEMPLATE" || fail "run_after_68 no longer probes the alert route"
