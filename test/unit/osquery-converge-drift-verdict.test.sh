...
function test_a_file_matching_on_content_mode_owner_and_group_has_not_drifted() {
  assert_equals ok "$(osquery_converge_file_verdict file 1 0644 0 0)"
}

function test_nothing_at_the_path_reads_as_absent() {
  assert_equals absent "$(osquery_converge_file_verdict absent '' '' '' '')"
}
...
function test_a_symlink_standing_where_the_config_belongs_reads_as_irregular() {
  ...
  assert_equals irregular "$(osquery_converge_file_verdict symlink 1 0644 0 0)"
}

function test_a_directory_standing_where_the_config_belongs_reads_as_irregular() {
  assert_equals irregular "$(osquery_converge_file_verdict other '' 0755 0 0)"
}

function test_differing_bytes_read_as_content_drift() {
  assert_equals content "$(osquery_converge_file_verdict file 0 0644 0 0)"
}
...
function test_correct_bytes_under_a_world_writable_mode_read_as_drift_not_as_ok() {
  ...
  assert_equals mode "$(osquery_converge_file_verdict file 1 0666 0 0)"
}

function test_correct_bytes_owned_by_a_non_root_user_read_as_drift() {
  assert_equals owner "$(osquery_converge_file_verdict file 1 0644 501 0)"
}

function test_correct_bytes_owned_by_a_non_wheel_group_read_as_drift() {
  assert_equals group "$(osquery_converge_file_verdict file 1 0644 0 20)"
}
...
function test_a_state_that_could_not_be_read_reads_as_unreadable_never_as_ok() {
  ...
  assert_equals unreadable "$(osquery_converge_file_verdict file 1 '' 0 0)"
  assert_equals unreadable "$(osquery_converge_file_verdict file '' 0644 0 0)"
}
...
function test_content_drift_is_reported_ahead_of_an_attribute_that_also_drifted() {
  ...
  assert_equals content "$(osquery_converge_file_verdict file 0 0666 501 20)"
}

# --- directories ------------------------------------------------------------

function test_a_directory_at_0755_root_wheel_has_not_drifted() {
  assert_equals ok "$(osquery_converge_directory_verdict directory 0755 0 0)"
}
...
function test_a_group_writable_directory_reads_as_drift() {
  ...
  assert_equals mode "$(osquery_converge_directory_verdict directory 0775 0 0)"
}

function test_a_missing_directory_reads_as_absent() {
  assert_equals absent "$(osquery_converge_directory_verdict absent '' '' '')"
}

function test_a_file_standing_where_the_packs_directory_belongs_reads_as_irregular() {
  assert_equals irregular "$(osquery_converge_directory_verdict file 0644 0 0)"
}

# --- the restart fold -------------------------------------------------------

function test_nothing_drifted_means_no_restart() {
  assert_equals no-restart "$(osquery_converge_restart_verdict ok ok ok)"
}

function test_any_single_drifted_path_warrants_a_restart() {
  assert_equals restart "$(osquery_converge_restart_verdict ok mode ok)"
}
...
function test_a_drifted_path_in_the_last_position_still_warrants_a_restart() {
  ...
  assert_equals restart "$(osquery_converge_restart_verdict ok ok content)"
}

function test_an_empty_verdict_list_means_no_restart() {
  # Nothing was examined, so nothing justifies bouncing the root daemon.
  assert_equals no-restart "$(osquery_converge_restart_verdict)"
}
