...
# (~/.local/libexec/unattended-upgrades/homebrew-weekly-upgrade.sh) -- the exact artifact launchd runs
...
grep -qF '.local/libexec/unattended-upgrades/homebrew-weekly-upgrade.sh' <<<"$body" ||
  fail "recipe does not invoke the deployed ~/.local/libexec/unattended-upgrades/homebrew-weekly-upgrade.sh"

if grep -qF 'dot_local/libexec/unattended-upgrades/executable_homebrew-weekly-upgrade.sh' <<<"$body"; then
  printf 'recipe body:\n%s\n' "$body" >&2
  fail "recipe invokes the repo SOURCE copy instead of the deployed one"
fi

printf 'homebrew-brew-upgrade-recipe: OK (runs the deployed ~/.local/libexec/unattended-upgrades/homebrew-weekly-upgrade.sh)\n'
