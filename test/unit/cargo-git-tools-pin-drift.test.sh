...
# re-fetches) on every apply, and one that always matches means a `ref` bump
# never lands and dam stays stale forever, silently. Both failures are
# invisible in an apply, so already_installed's own contract is pinned here
# directly against .crates.toml, no cargo spawn needed.
#
...
# Runs already_installed <repo> <rev> as a fresh process (never `source`, so a
# `set -e` in the snippet cannot terminate the test runner) and returns its
# exit code as this function's own.
run_already_installed() {
  local driver="$fixture_home/driver.sh"
  cp "$snippet" "$driver"
  printf 'already_installed %q %q\n' "$1" "$2" >>"$driver"
  HOME="$fixture_home" bash "$driver"
}

function test_a_crates_toml_line_at_the_pinned_rev_is_already_installed() {
...
EOF
  run_already_installed "webdavis/damnit" "dfe31427f7bc41a289aa0b48f9c3a75eb7689a0a"
...
"damnit 0.2.0 (git+https://github.com/webdavis/damnit?rev=dfe31427f7bc41a289aa0b48f9c3a75eb7689a0a#dfe31427f7bc41a289aa0b48f9c3a75eb7689a0a)" = ["dam"]
EOF
  run_already_installed "webdavis/damnit" "0000000000000000000000000000000000000000"
