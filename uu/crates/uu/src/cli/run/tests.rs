...
fn a_missing_registration_refuses_before_any_command_or_diagnostic_can_run() {
    ...
    assert_eq!(
        error.detail(),
        "lane `chosen` has type `fixture-command`, which is no lane type; this build serves brew, cargo, claude-plugins, command, herdr, npm, nvim-mason, nvim-parsers, nvim-plugins, nvim-smoke-test, skills, uv"
    );
