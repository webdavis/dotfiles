...
fn a_child_that_exits_the_deferred_code_is_recorded_deferred_not_failed() {
    ...
    assert!(
        report.verdict() == uu_domain::LaneVerdict::Deferred,
        "{report:?}"
    );
    assert_eq!(
        report.last_failure(),
...
fn a_deferred_lines_reason_is_the_childs_own_text_not_a_fixed_string() {
    ...
    let program = "/usr/local/bin/updater";
    let runner = ScriptedRunner::new(&[]).deferring_because(&[program], "UNIQUE-DEFER-REASON-42");
    let lane = command_lane(&[program]);
    let report = lane.run("mine", &stub_facts(), &runner);
    assert!(
        report
...
#[test]
fn a_pending_command_preserves_its_stdout_and_own_reason_without_failure_or_deferral() {
    let program = "/fixture/pending-updater";
    let runner = ScriptedRunner::new(&[])
        .answering("two updates waiting\n")
        .pending_because(&[program], "UNIQUE-PENDING-REASON-43");
    ...
    assert!(
        report
            .lines
            .iter()
            .any(|line| line == "two updates waiting")
    );
    assert!(
        report
            .lines
            .iter()
            .any(|line| line.contains("pending") && line.contains("UNIQUE-PENDING-REASON-43"))
    );
}
