...
1. **List available subtitles** - see what's actually available
1. **Try manual subtitles first** (`--write-sub`) - highest quality
1. **Fallback to auto-generated** (`--write-auto-sub`) - usually available
1. **Last resort: Whisper transcription** - if no subtitles exist (requires user confirmation)
1. **Confirm the download** and show the user where the file is saved
1. **Optionally clean up** the VTT format if the user wants plain text
...
**If installation fails**: Inform the user they need to install yt-dlp manually and provide them with
installation instructions from https://github.com/yt-dlp/yt-dlp#installation
...
**IMPORTANT**: Display the file size to the user and ask: "No subtitles are available. I can download the
audio (approximately X MB) and transcribe it using Whisper. Would you like to proceed?"
...
If not installed, ask user: "Whisper is not installed. Install it with `pip install openai-whisper`
...
After transcription completes, ask user: "Transcription complete! Would you like me to delete the audio
file to save space?"
...
Use this to create meaningful filenames based on the video title. Clean the title for filesystem
compatibility:
...
YouTube's auto-generated VTT files contain **duplicate lines** because captions are shown progressively
with overlapping timestamps. Always deduplicate when converting to plain text while preserving the
original speaking order.
...
**Note**: This complete workflow handles all scenarios with proper error checking and user prompts at
each decision point.
