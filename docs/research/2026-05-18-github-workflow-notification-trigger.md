...
- **The existing GHA watcher (commit `a6a1df2`, 2026-04-25) is a 60-second poller** that uses
  ...
  completed-but-newly-seen run. ~600–1000 API calls/hour against a 5000/hour limit — workable but not
  free.
...
- **`flock(1)` is NOT shipped with macOS.** Empirically verified — `command -v flock` returns empty on
  the user's machine. The `flock` CLI requires `brew install util-linux`. **launchd `QueueDirectories` is
  the macOS-native queue** mechanism and pairs with the user's existing five `com.webdavis.*.plist.tmpl`
  LaunchAgents.
...
- **Agent 1 — gh CLI native surface + GitHub trigger mechanisms.** Verified `gh` version locally;
  surveyed 7 mechanisms (workflow_run webhook, polling, push-from-workflow, GraphQL, email-to-action,
  step summary, repository_dispatch); read the user's existing `gha-watcher.sh` + `gha-notify.sh` + plist
  to characterize the current pattern accurately.
...
to the actor) and couples notifications to email-delivery latency (1–5 minutes).
...
**Tailscale Funnel (recommended).** One command — `tailscale funnel 8080` — exposes a localhost service
on a public `*.ts.net` URL with valid TLS certs, no DNS setup, no cert renewal. Funnel is opt-in per-node
...
URL is more reusable than a `ts.net` URL. Setup: 15–30 minutes.
...
**Effort:** 4/5. Cloudflare Tunnel setup is +15–30 min (DNS record, named tunnel, `cloudflared.yml`,
launchd unit).
...
1. **Sink failure isolation.** If the Hue bridge is down or `mouse` is offline, only that one sink fails
   — alerter still fires, and the queue entry can be marked attempted-and-failed without blocking the
   next event.
...
documented public endpoint. This isn't an oversight — it's GitHub's deliberate architecture, optimized
for at-scale fan-out via webhooks rather than long-lived client subscriptions.
...
### Next Steps (after the audit cycle's Setup S1–S4)
...
1. **OpenClaw hook concurrency** — if Stephen plans heavy use of `/hooks/notify`, source-code
   verification of the hook handler's queue interaction is worth one focused session. Not a blocker.
...
[6] Tailscale Funnel — public HTTPS for tailnet services. https://tailscale.com/kb/1247/funnel
(Retrieved: 2026-05-18)
...
- **Phase 3 (RETRIEVE):** Agents ran 5–10 minutes each. Combined wall time ~12 minutes for all three
  (parallel execution).
...
- **Phase 5 (SYNTHESIZE):** Three patterns identified — chokepoint earns keep three times over; gh
  follows polling-or-webhook dichotomy; latent infrastructure shapes recommendation.
