...
| Rendered starts  | Five starts through a real pseudo-terminal on 2026-09-15, each with a UI attached (`#nvim_list_uis() == 1`) and zero error lines outside one terminal artifact, classified below.                        |
| Rendered loading | `Lazy! load all` in each rendered start reported 99 declared and 99 loaded, so every plugin loaded with a UI present rather than only the VeryLazy set.                                                  |
| Rendered snacks  | With a UI attached, `Snacks.input`, `Snacks.picker` and `Snacks.notifier` are all present. `Snacks.select` is absent because snacks ships no such module; select comes from the picker.                  |
...
| Snacks input/select/setup errors                                     | ANSWERED 2026-09-15 by the rendered runs above. `input`, `picker` and `notifier` are present with a UI attached, and `Snacks.select` was never a module: `~/.local/share/nvim/lazy/snacks.nvim/lua/snacks/` has no `select`, so the headless note was reading an absence that is correct. |
...
| Terminal DSR notice in rendered starts                               | `E1568: Terminal did not respond to DSR request for 'background' color` appears once per rendered start. It comes from running the editor under `script(1)`, which answers no Device Status Report, and it is the only message in any of the five runs. Not a configuration defect.       |
...
- Rendered STARTUP is done (see the rendered rows above). What remains of this bullet is which-key
  NAVIGATION and buffer-local key interactions, both of which need a person pressing keys and reading the
  popup. The group names themselves are reconciled: `lua/plugins/which-key.lua` declares `<leader>x` as
  `xcode` and `<leader>d` as `docker`, so the record's former `X = xcode` and `d = do` expectations were
