...
recap takes `DEFAULT_ROUTE` like every other session event, because the `pns-recap` route and its channel
retired on 2026-09-15. pns keeps a ROSTER of the routes it posts to, `pns_domain::routes::ROUTES`,
because each one is signed with its own key and a route with no key is a post pns refuses rather than
signs with somebody else's. So adding a route pns posts to is two registrations, the gateway and that
roster, and a route only uu or posture posts to needs the gateway alone.
...
`{body}`); `posture-pages` and `uu-runs` keep the event-shaped prompt they already had.

**Every secret and every channel id comes from KeePassXC by entry name, two entries per route.** Ten
...
401 on four of the five routes, so apply once, with KeePassXC unlocked, then `hermes gateway restart`.
...
`general`, `pns-events`, `posture-pages`, `priority`, `uu-runs`. What to expect from the apply itself:
`run_after_68` prints NOTHING when all five routes are present with a secret, a snowflake `chat_id` and
