...
git checkout -b integration/modernization   # branches at e8094b5, the current head
```
...
- [ ] **P-8: Operator review gate.** Stop. The operator reviews and merges (squash per house
  `pr-merge`). If the slice needs a KeePassXC-gated apply, the operator does it here. Do not start the
  next slice until this PR is merged — the next slice branches from the main it created.
