...
- Plugin id: `webdavis.last-workspace`; action id: `last_workspace` (mirrors herdr's `last_pane`).
...
**Files:**
- Create: `dot_local/share/herdr/plugins/last-workspace/Cargo.toml`
- Create: `dot_local/share/herdr/plugins/last-workspace/src/main.rs`
- Create: `dot_local/share/herdr/plugins/last-workspace/Cargo.lock` (generated by the build)
...
Run: `cd ~/.local/share/herdr/plugins/last-workspace 2>/dev/null || cd dot_local/share/herdr/plugins/last-workspace; cargo test`
...
git add dot_local/share/herdr/plugins/last-workspace/Cargo.toml \
        dot_local/share/herdr/plugins/last-workspace/Cargo.lock \
        dot_local/share/herdr/plugins/last-workspace/src/main.rs
...
**Files:**
- Create: `dot_local/share/herdr/plugins/last-workspace/herdr-plugin.toml`
...
```bash
git add dot_local/share/herdr/plugins/last-workspace/herdr-plugin.toml
...
**Interfaces:**
- Consumes: the plugin source (Tasks 1–2) at `~/.local/share/herdr/plugins/last-workspace/`.
...
#   {{ include "dot_local/share/herdr/plugins/last-workspace/src/main.rs" | sha256sum }}
#   {{ include "dot_local/share/herdr/plugins/last-workspace/Cargo.lock" | sha256sum }}
#   {{ include "dot_local/share/herdr/plugins/last-workspace/herdr-plugin.toml" | sha256sum }}

plugin_dir="$HOME/.local/share/herdr/plugins/last-workspace"
plugin_id="webdavis.last-workspace"
...
**Interfaces:**
- Consumes: the `webdavis.last-workspace.last_workspace` action (Task 2).
...
# positional — so the webdavis.last-workspace plugin tracks workspace focus via
...
key = "prefix+ctrl+\\"
type = "plugin_action"
command = "webdavis.last-workspace.last_workspace"
...
chezmoi apply --force "$HOME/.local/share/herdr/plugins/last-workspace" \
...
```
Expected: build succeeds; "linked herdr plugin webdavis.last-workspace" (or "already linked"); state seeded.
...
```bash
mru="${XDG_STATE_HOME:-$HOME/.local/state}/herdr/plugins/webdavis.last-workspace/mru"
foc() { herdr workspace list 2>/dev/null | jq -r '.result.workspaces[]|select(.focused)|.label'; }
inv() { herdr plugin action invoke last_workspace --plugin webdavis.last-workspace >/dev/null 2>&1; }
...
Run: `(cd dot_local/share/herdr/plugins/last-workspace && cargo test) && just l && git status --short`
