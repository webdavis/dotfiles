...
(`src/main.rs:156-159`), and Cargo's `0.1.0` is never emitted anywhere. The second consumer of
`--elapsed` is `webdavis/pns.nvim`, the editor-side producer in its own repository, which carries no
thresholds and gates itself on a minimum pns version (nvim overhaul task 26, blocked on this PR), so
that handshake needs a version to read. Three things this PR states rather than assumes: the word
