...
| T2    | `neotest-bashunit`: the adapter, which SHIPPED as its own repository at `webdavis/neotest-bashunit` (operator ruling 2026-09-05: every custom Neovim plugin is its own GitHub repository installed the ordinary way, so the `dir =` staging step this row planned was skipped); headless Lua tests for discovery and result parsing over fixture JSON; a real-pane run against T1's test. Design first: the per-test JSON fields verified from `src/reports/json.sh` at the pin, how a failure maps to a line, what `--filter` matches, what a file with no tests returns | T1 |
