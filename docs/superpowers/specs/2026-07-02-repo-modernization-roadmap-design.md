...
**Status:** approved via brainstorming; SP1 to SP2 designed in full, **SP3 behavior contract approved,
final implementation spec pending** (contract below; open items enumerated in the deferred index),
SP4 to SP7 indexed. Extended with a complete feature inventory and a work ledger of 85+ items (≈40 known +
...
4. herdr migration (2 to 3 PRs: config; smart-nav plugin; bashrc/workspace integration).
...
closure-only (both pins re-verified fail-closed in a fresh session, on any drift, Gates 1 to 4 restart;
...
slices. Retirement happens in Gates 1 to 3 (approve / execute / verify), not Gate 5, so the topology soaked
...
| S1 to S4 (docs archive, CLAUDE.md audit rule, GH #5/#13 closures) | never executed; GH **#5 is now closed** (nushell NO-GO, decision 9) | SP7: re-rule S1/S2/S4; #13 (Zellij) still open |
...
- SP2: integration PR open + labeled; every slice PR green on lint/tests and merged after user review;
  ...
  of the final topology; **Gate 5** both pins re-verified, closure only, on any drift, Gates 1 to 4
...
| **phone-attention** | tailscale bytes→`mister` rate | is the Moshi app foregrounded | 0 B/s backgrounded (session still alive), ~150 B/s reading, 1 to 5 KB/s typing, iOS suspends the app in background, so rate ≈ foreground. |
...
gap-ladder test (calls after 30 s/60 s/2 m/5 m/10 m/15 m idle) showed **no decay**, 55 to 251 ms at every
...
- **Surfaces:** at-desk → banner (whitelist the notifier app in every Focus mode **except Sleep**; Sleep
  ...
  clickable push per still-blocked session** (0 to 2 items, each routes to its pane), heals missed Focus/
...
- **Inventory:** every subsystem A to P traces to real files (spot-checked by a completeness critic; the
