...
This is the deliberate difference from the Rust five-crate layout. Swift's `internal` default already
confines a symbol to its module, so a folder split is free and a target split would add build
configuration for a boundary the language is already keeping.
