...
machine). It talks to `HERDR_SOCKET_PATH` directly, one connection and two newline-delimited JSON
requests per jump, and falls back to the `herdr` CLI through `HERDR_BIN_PATH` when the socket is absent
