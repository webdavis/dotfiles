...
gateway's routes are written in. Notice and Info are `posture`, the pipeline's own channel. No tier at
all names no route, and the sink then keeps the route it was configured with, which is how the heartbeat,
the daily digest, the poll and funnel pages and the cursor-reset warning all keep the route their command
built. The watchdog is unchanged: it still probes `priority`, because that is the route whose health it
reports on.
