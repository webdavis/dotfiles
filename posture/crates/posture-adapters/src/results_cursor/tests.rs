...
/// A cursor path in a directory that removes itself. Hold the sandbox for as
/// long as the path is used; dropping it early takes the cursor with it.
fn scratch() -> (Sandbox, PathBuf) {
    let sandbox = Sandbox::new("cursor");
    let path = sandbox.join("offset");
    (sandbox, path)
}

#[test]
fn a_cursor_survives_the_round_trip_it_is_written_for() {
    let (_sandbox, path) = scratch();
...
fn asking_twice_in_one_run_is_still_held_rather_than_refused() {
    ...
    let (_sandbox, cursor) = scratch();
    let lock = SingleRunLock::beside(&cursor);
    assert!(lock.taken());
    assert!(lock.taken());
}
...
fn a_lock_whose_directory_cannot_be_made_refuses_rather_than_running_unlocked() {
    ...
    let sandbox = Sandbox::new("lock-file");
