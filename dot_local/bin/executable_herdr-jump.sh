...
# workspace list` already emits JSON; it has no --json flag. Guard the pipeline
# so a down/erroring herdr server (non-zero rc, or an error-JSON envelope that
# makes jq choke) yields a clean one-line error instead of a bare set -e abort
# or jq spew. An empty result (label simply absent) is a success -> create path.
if ! id=$("$herdr_bin" workspace list 2>/dev/null |
  jq -r --arg l "$label" '[.result.workspaces[] | select(.label == $l) | .workspace_id][0] // empty' 2>/dev/null); then
  printf '%s: could not query herdr workspaces (is the herdr server running?)\n' "${0##*/}" >&2
  exit 1
fi
