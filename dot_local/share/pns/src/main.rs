...
fn replay_missed(
    ...
    pns_application::replay_missed::ReplayMissedNotifications {
        moment: &catch_up,
        activity: &catch_up,
        publisher: &catch_up,
        delivery: &catch_up,
    ...
    .run(
        decision,
        pns_application::replay_missed::RecapPolicy {
...
impl pns_application::ports::records::ReturnMoment for CatchUp<'_> {
    fn claim(
        &self,
        now: Option<u64>,
        take_journal: bool,
...
impl pns_application::ports::records::ActivityRing for CatchUp<'_> {
...
impl pns_application::ports::delivery::RecapPublisher for CatchUp<'_> {
...
impl pns_application::ports::delivery::ReplayDelivery for CatchUp<'_> {
...
fn presence_snapshot<R: pns::system::CommandRunner>(
...
fn last_narrowing(state: &Path) -> Option<pns::presence_journal::Entry> {
    let contents =
...
/// the refusal.
fn presence_settings() -> Option<pns::config::Presence> {
    match load_config(&config_path(&std::env::var("HOME").unwrap_or_default())) {
...
mod tests {
    ...
    const ROOM_BODY: &str = r#"{"data":[{"id":"studio","metadata":{"name":"3F - Studio"}}]}"#;

    /// A bridge that serves what it was given and nothing else, so a test can
    /// take either listing away.
    struct PollBridge(Vec<(&'static str, &'static str)>);
    ...
    fn at_the_desk(status: pns::presence::PresenceStatus) -> pns::presence_policy::Snapshot {
    ...
    fn in_the_kitchen() -> pns::presence_policy::Snapshot {
        pns::presence_policy::Snapshot {
            desk_idle_secs: None,
