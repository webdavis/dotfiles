...
- Thresholds: the near edge is EXCLUSIVE, the far edge INCLUSIVE. An entry stamped exactly at `since` is
  ...
  exactly that size is read, one byte more is `FileTooLarge` (`src/main.rs:readable_ring`).
