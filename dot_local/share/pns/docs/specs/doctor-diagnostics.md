...
- Failure sources: `src/main.rs:disabled_backend_warnings` emits one line per switched-off `router` or
  ...
  bad table name, `` pns: config error (`presence` is enabled and needs `hue`, which is not); running
  every built-in plugin `` for a borrowed credential nobody switched on, and
