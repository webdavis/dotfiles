...
pub fn condense(reply: &str) -> (String, String) {
    ...
    let Some(home) = condenser_home(&user_home, std::env::var("PNS_CODEX_HOME").ok().as_deref())
    else {
        return fallback();
    };
...
fn condenser_home(user_home: &str, home_override: Option<&str>) -> Option<std::path::PathBuf> {
