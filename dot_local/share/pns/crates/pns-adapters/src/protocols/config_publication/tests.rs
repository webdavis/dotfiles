use super::backup::keep_aside_at;
use super::*;
...
fn a_same_second_backup_collision_names_the_backup_it_could_not_claim() {
    ...
    std::fs::create_dir_all(path.parent().expect("the directory")).expect("the directory");
    std::fs::write(&path, "# the one it replaces\n").expect("the config");
    let claimed = backup_path(&path, FIXED_EPOCH).expect("the backup name");
...
fn a_directory_at_the_config_path_is_named_rather_than_the_backup_it_could_not_replace() {
    ...
    let refusal = keep_aside(&path).expect_err("a directory cannot be renamed onto a plain file");
    assert!(
...
#[test]
fn a_failed_forced_publication_restores_the_previous_config() {
    let state = scratch("setup-restore-publish");
    ...
    let file = std::fs::OpenOptions::new()
        ...
        .open(&pending)
        .unwrap();
    std::fs::rename(&pending, state.join("withdrawn-pending")).unwrap();
    std::fs::write(&path, "old config").unwrap();
    let refusal = write_then_publish(&path, &pending, file, "new config", true).unwrap_err();
    assert_eq!(
        std::fs::read_to_string(&path).ok().as_deref(),
        Some("old config"),
        "failed publication must restore the old config"
    );
