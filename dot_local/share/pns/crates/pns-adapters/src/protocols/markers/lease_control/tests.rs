mod tests {
    use crate::FileLoopLeases;
    use crate::marker_files::{renew_loop_lease, sweep_leases};
    use pns_application::AcquireLoopLease;
    use std::path::Path;
    fn end_lease(state: &Path, pane: &str) -> Result<(), String> {
        AcquireLoopLease {
            leases: &FileLoopLeases::new(state.to_path_buf()),
        }
        .end(pane)
    }
    ...
    fn a_lease_is_renewed_only_while_it_exists_and_swept_once_it_times_out() {
        ...
        let marker = crate::marker_files::lease_marker(&state, "wW:p21")
            .expect("herdr's own id names a lease");
        std::fs::create_dir_all(crate::marker_files::lease_dir(&state))
    ...
    fn a_renewal_writes_through_the_lease_it_found_rather_than_publishing_a_new_one() {
        ...
        let marker = crate::marker_files::lease_marker(&state, "wW:p21").expect("herdr's own id");
        std::fs::create_dir_all(crate::marker_files::lease_dir(&state))
            .expect("the lease directory");
        std::fs::write(&marker, "1000\n").expect("a lease taken by hand");
