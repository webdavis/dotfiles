...
pub fn read_news(state: &Path) -> pns_domain::lights::unread::News {
...
fn news_at(path: &Path) -> pns_domain::lights::unread::News {
    std::fs::read_to_string(path)
        .ok()
        .and_then(|line| crate::lights_codec::parse_news(&line))
        .unwrap_or_default()
}
...
pub fn record_news(
    ...
    // make safe.
    if pns_domain::lights::unread::news_after(
        pns_domain::lights::unread::News::default(),
        behaviour,
        now,
    )
    .is_none()
    ...
    if let Some(next) = pns_domain::lights::unread::news_after(held, behaviour, now) {
        ...
        let _ = publish_state_line(&path, &crate::lights_codec::render_news(&next));
    }
    if claimed {
...
/// cap either.
pub(super) const LIGHTS_NEWS: &str = "lights-news";
