...
fn a_held_records_phase_round_trips_through_remember_held_and_read_held() {
    ...
    let phased = pns_domain::lights::phase::HeldEntry {
...
fn a_bare_token_on_disk_still_reads_as_a_held_lamp_with_no_phase() {
    ...
    assert_eq!(
        read_held(&state),
        Some(vec![pns_domain::lights::phase::HeldEntry::bare(LAMP_PATH)])
    );
...
fn the_news_record_is_written_for_a_finished_or_a_dead_turn_and_read_back_as_it_was() {
    ...
    record_news(
        &state,
        pns_domain::lamps::config::Behaviour::Blocked,
        Some(1_400),
    );
    assert_eq!(
    ...
    record_news(&state, pns_domain::lamps::config::Behaviour::Done, None);
