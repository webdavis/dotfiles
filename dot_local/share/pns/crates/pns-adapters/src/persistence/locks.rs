...
pub fn claim_lock(lock: &Path, now: u64, stale_secs: u64) -> bool {
...
fn lock_aged_out(lock: &Path, now: u64, stale_secs: u64) -> bool {
    std::fs::symlink_metadata(lock)
        .ok()
        .as_ref()
        .and_then(|metadata| metadata.modified().ok()?.duration_since(UNIX_EPOCH).ok())
...
pub struct HeldLock(pub std::path::PathBuf);
