mod tests {
    ...
    use pns_application::RunDaemonTick;
    ...
    fn a_job_waits_while_its_own_child_lives_and_fires_once_that_child_has_gone() {
        ...
        let spool = crate::job_spool::spool_dir(&state);
        ...
        crate::job_spool::hand_back(&spool, &job).expect("the record lands");
        ...
        let mut children = DaemonChildren {
        ...
        RunDaemonTick {
            spool: &crate::FileJobSpool::new(state.clone()),
            children: &mut children,
        }
        .run(Some(200), &mut reported, |_| {});
        assert_eq!(
        ...
        assert_eq!(
            ...
            1,
            "and the live child was not reaped"
        );
        assert_eq!(
        ...
        let _ = children.children[0].child.kill();
        let _ = children.children[0].child.wait();
        RunDaemonTick {
            spool: &crate::FileJobSpool::new(state.clone()),
            children: &mut children,
        }
        ...
        for bounded in &mut children.children {
