...
impl<B: Bridge> HuePulse<B> {
    ...
    pub fn run(&self, behaviour: pns_domain::lamps::config::Behaviour) -> usize {
...
pub fn signal_fixtures<B: Bridge>(
    ...
    let (signal, color) = match behaviour {
        pns_domain::lamps::config::Behaviour::Done => {
            ("on_off_color", pns_domain::pulse::SUCCESS_COLOR)
        }
        pns_domain::lamps::config::Behaviour::Failed => {
            ("on_off_color", pns_domain::pulse::FAILURE_COLOR)
        }
        _ => return 0,
    };
