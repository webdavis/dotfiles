mod tests {
    ...
    fn a_poll_publishes_the_room_it_read_as_the_line_the_sensor_parses() {
        ...
        let published = std::fs::read_to_string(state.join(pns_adapters::PRESENCE_STATE_FILE))
        ...
        assert_eq!(
            ...
            Some(pns_domain::RawPresence {
                poll_epoch: 1_788_456_100,
                edge: Some(pns_domain::Edge {
                    epoch: 1_788_456_009,
                    motion: false,
        ...
        assert_eq!(
            published_mode(&state.join(pns_adapters::PRESENCE_STATE_FILE)),
            0o600
        );
    }
    ...
    fn a_bridge_that_did_not_answer_leaves_the_last_reading_where_it_was() {
        ...
        let reading = state.join(pns_adapters::PRESENCE_STATE_FILE);
