use crate::{LampBridge, LampWrite};
...
pub struct Breathing {
    ...
    pub held: pns_domain::lights::held::Held,
    ...
    pub cycle: Vec<pns_domain::lights::breath::Leg>,
    pub color: pns_domain::pulse::PulseColor,
    pub resume: pns_domain::lights::breath::Resume,
}
...
pub fn drive_breaths<B: LampBridge>(
    bridge: &B,
    budget_ms: u64,
    breathing: &[Breathing],
    ...
    let mut schedule: Vec<(
        pns_domain::lights::breath::Fade,
        &Breathing,
        Option<pns_domain::pulse::PulseColor>,
    )> = Vec::new();
    for entry in breathing {
        let fades = pns_domain::lights::breath::breath_fades(budget_ms, &entry.cycle, entry.resume);
        for (index, fade) in fades.iter().enumerate() {
            ...
            let body = if index == 0 { Some(entry.color) } else { None };
    ...
    for (fade, entry, body) in schedule {
        ...
        bridge.write(&entry.path, &LampWrite::Fade { fade, color: body });
