...
| `RunDelivery`       | `delivery::EngineRunDelivery`          | Configured alert process and signed records through existing clients.                                              |
...
private child files. `config.rs` owns the shared file boundary, top-level configuration and record
settings. Size evidence is measured from the formatted source with the Rust standard's physical-line
command.
...
named successors; four registration cases brought that row to 304. Pending records and escalation
preserve those cases and add 24, for 328 named cases. The two command timing cases move into the actual
command composition, and the ten staleness cases split between streak policy and persistence/delivery
failures. The original record-body and delivery-policy moves remain recorded in the map.
...
The pending and escalation additions retain the predecessor's successful named cases. Each addition has a
passing outcome below one second and a real source fault that fails its intended assertion.
