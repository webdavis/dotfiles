...
impl SkillsCandidate {
    pub fn validate(&self, roster: &SkillsRoster, mode: SkillsBuildMode) -> Result<(), String> {
        match self.validate_content(roster, mode) {
            Ok(()) => Ok(()),
            Err(why) => {
    ...
    fn validate_content(&self, roster: &SkillsRoster, mode: SkillsBuildMode) -> Result<(), String> {
        let lock = self.agents().join(".skill-lock.json");
        if !regular(&lock) {
