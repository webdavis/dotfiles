...
# The npx `skills` CLI is PINNED rather than @latest: this runs unattended and
# installs code that agents then execute, so a compromised publish would land
# here with no review. Bump deliberately after reading the release; `npm view
# skills version` shows the current upstream. (Pinning the CLI does not pin the
# SKILL CONTENT it fetches, which still tracks each upstream's main; per-skill
# pinning is architectural and belongs to the SP9 Rust port.)
...
# helpers/log-entries.sh for the entry shape and why it exists). The success
...
__gen_lane_npx() {
  ...
  for repo in "${repos[@]:-}"; do
    ...
    if npx --yes "skills@$SKILLS_CLI_VERSION" add "$repo" "${skill_args[@]}" \
...
if [[ $DRYRUN == "--dry-run" ]]; then
  ...
  if [[ $__update_skills_lock_rc -eq 75 ]]; then
    ...
    if __update_skills_scheduled_budget_exhausted; then
      __update_skills_contention_detail="another update-skills run held the serialize lock on the LAST scheduled slot this week, so this week's update never ran. No later slot remains: re-run ~/.local/libexec/unattended-upgrades/agent-skills/update-skills.sh by hand."
      ...
      __update_skills_alert "Weekly skills update deferred over lock contention on the LAST scheduled slot this week; no later slot remains, so it did not run. Re-run ~/.local/libexec/unattended-upgrades/agent-skills/update-skills.sh."
    fi
    ...
    __update_skills_alert "update-skills could not acquire its serialize lock (rc $__update_skills_lock_rc). Check that ~/.agents is writable, then re-run ~/.local/libexec/unattended-upgrades/agent-skills/update-skills.sh."
