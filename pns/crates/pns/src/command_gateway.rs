...
/// `pns gateway <verb>`: the daemon's own launchd service, wrapped the way
/// `hermes gateway` wraps its own.
///
/// A BARE `pns gateway` IS A REFUSAL, per the house rule that an unknown
/// argument never falls through to help with exit 0.
...
pub(crate) const GATEWAY_USAGE: &str = "pns: usage: pns gateway start | pns gateway stop | \
pns gateway restart | pns gateway status";
...
fn run_gateway(
    ...
    let Some(label) = label else {
        eprintln!("pns gateway: `[daemon] service` is not set");
        return 2;
    };
    match verb {
...
/// `[daemon] service`, or `None` for an absent key, an absent config, or one
...
fn service_label(home: &str) -> Option<String> {
    match pns_adapters::load_config(&pns_adapters::config_path(home)) {
        Ok(pns_adapters::LoadOutcome::Loaded(config)) => config.daemon_service,
        _ => None,
    }
}
