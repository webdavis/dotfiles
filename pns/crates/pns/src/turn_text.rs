...
pub(crate) fn turn_reply(payload: &HookPayload) -> String {
    let flatten = |text: &str| pns_domain::render::flatten_reply(text, REPLY_MAX_CHARS);
...
fn reread_interval() -> Duration {
    reread_interval_from(std::env::var(REREAD_INTERVAL_VARIABLE).ok().as_deref())
}
...
/// The interval, as the same `<count><ms|s|m|h>` string every flag and config
/// key takes, reported and dropped for the default when it will not parse.
///
/// THE RANGE IS THE VALIDATION, and it replaced a float parse whose own
/// checks looked complete: NaN, infinity and negatives were refused, but a
...
fn reread_interval_from(raw: Option<&str>) -> Duration {
    raw.and_then(|raw| {
        pns_domain::duration::parse_duration(REREAD_INTERVAL_VARIABLE, raw, REREAD_INTERVAL_RANGE)
...
/// sit re-reading a transcript that is never going to fill, so neither is
/// believed past its own: a stray zero in either costs seconds, never hours.
/// The attempt count is clamped to its ceiling and the interval is refused
/// above its own, each in the grammar its variable is typed in.
const MAX_REREAD_ATTEMPTS: u32 = 10;
const MAX_REREAD_INTERVAL: Duration = Duration::from_secs(5);

/// No wait at all is a legitimate setting, so the floor is zero and the
/// ceiling is the cap above.
const REREAD_INTERVAL_RANGE: std::ops::RangeInclusive<Duration> =
    Duration::ZERO..=MAX_REREAD_INTERVAL;
