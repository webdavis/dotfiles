...
fn a_section_over_no_contents_says_no_decision_has_been_recorded_and_names_nothing() {
    ...
    assert_eq!(
        spoken(None, Some(1_756_500_000)),
        vec![
    ...
    assert_eq!(spoken(Some(""), None), spoken(None, None));
    assert_eq!(spoken(Some("\n\n"), None), spoken(None, None));
}
...
const HEADING_TAIL: &str = ", every input behind them";
...
fn a_section_renders_the_newest_entry_first_capped_at_the_kept_count_with_each_ones_age() {
    ...
    assert_eq!(
        spoken(Some(SEVEN), Some(1_756_500_000)),
        vec![
...
fn a_section_counts_the_entries_it_actually_shows_rather_than_the_cap() {
    ...
    assert_eq!(
        spoken(Some(SEVEN), Some(1_756_500_000)).len(),
        KEPT + 1,
...
fn a_section_quotes_an_entry_it_cannot_read_and_still_renders_its_readable_neighbours() {
    ...
    assert_eq!(
        spoken(Some(mixed), Some(1_756_500_000)),
        vec![
...
fn an_unreadable_entry_is_quoted_short_and_with_its_control_bytes_escaped() {
    ...
    let rendered = spoken(Some("\u{1b}[31mred\tand\u{7}long\n"), Some(1_756_500_000));
    assert_eq!(
    ...
    let long = format!("{}\n", "z".repeat(500));
    assert_eq!(
        spoken(Some(&long), Some(1_756_500_000))[1],
...
fn a_section_invents_no_age_for_an_entry_or_a_reader_that_had_no_clock() {
    ...
    assert_eq!(
        spoken(Some("- a/one surface=Away\n"), Some(1_756_500_000)),
        vec![
            format!("pns doctor: the last decision{HEADING_TAIL}"),
...
/// The section's lines alone, since every assertion here is about the section's
/// SHAPE (order, cap, ages, escaping) rather than about which mark a row
/// carries.
///
/// RUN ON `--raw`, where one entry is still one line. The spoken form turns an
/// entry into a sentence and sometimes a second line under it, which is a
/// different thing to test and has its own tests beside the summariser.
fn spoken(contents: Option<&str>, now: Option<u64>) -> Vec<String> {
    pns_domain::doctor::decision_section(contents, now, pns_domain::doctor::Detail::Raw)
        .into_iter()
        .map(|(_, line)| line)
        .collect()
}
