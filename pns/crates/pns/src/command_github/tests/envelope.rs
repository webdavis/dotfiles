mod tests {
    use super::super::github_test_fixture::event;
    use super::super::*;
    ...
    fn the_envelope_names_the_repository_as_the_project_and_carries_the_extension() {
        let request = request_for(&event("webdavis/dotfiles", "id-1")).expect("it is spellable");
        assert_eq!(
    ...
    fn the_request_id_is_the_events_identity_so_the_ledger_is_a_second_guard() {
        ...
        assert_eq!(
            request_for(&one).map(|request| request.request_id.as_str().to_string()),
    ...
    fn a_mapped_repository_reaches_its_own_channel_and_an_unmapped_one_the_catch_all() {
        ...
        .collect();
        let landed = |repo: &str| {
            let request = request_for(&event(repo, "id-1")).expect("it is spellable");
