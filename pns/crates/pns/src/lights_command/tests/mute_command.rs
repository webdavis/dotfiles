...
fn a_duration_outside_the_bounds_is_refused_by_what_was_typed() {
    ...
    for typed in ["30", "", "1d", " 5m"] {
        assert_eq!(
            mute_command(&typed_at("3F - Studio", typed), &known, ONE_HOUR),
            Err(format!(
                "pns: mute duration {typed:?} is not <count><ms|s|m|h>"
            )),
            "typed: {typed:?}"
        );
    }
    assert_eq!(
        mute_command(&typed_at("3F - Studio", "30m"), &known, ONE_HOUR),
        Ok(MuteCommand::Mute {
...
fn a_place_the_config_does_not_name_is_refused_rather_than_silently_stored() {
    ...
    assert_eq!(
        mute_command(&typed_at("3F - Nowhere", "30m"), &known, ONE_HOUR),
        Err(
            "pns: lights mute: \"3F - Nowhere\" is no lamp, room or zone \
             this can mute; a mute reaches \"3F - Studio\", \
             \"3F - Studio - HCL3\""
                .to_string()
        ),
    ...
    assert_eq!(
        mute_command(&typed_at("3f - studio", "30m"), &known, ONE_HOUR),
        Err(
            "pns: lights mute: \"3f - studio\" is no lamp, room or zone \
             this can mute; a mute reaches \"3F - Studio\", \
             \"3F - Studio - HCL3\""
                .to_string()
        ),
    ...
    assert_eq!(
        mute_command(&typed_at("3F - Studio - HCL3", "30m"), &known, ONE_HOUR),
        Ok(MuteCommand::Mute {
    ...
    assert_eq!(
        mute_command(&typed_at("3F - Nowhere", "off"), &known, ONE_HOUR),
        Ok(MuteCommand::Unmute {
    ...
    assert_eq!(
        ...
        Err(
            "pns: lights mute: \"3F - Studio - HCL1\" is no lamp, room or zone \
             this can mute; this config claims no lamp at all, so there is \
    ...
    assert_eq!(
        mute_command(&arguments, &known, ONE_HOUR),
        Err(
            "pns: lights mute takes a place, optionally with a duration \
...
fn a_bare_mute_lasts_until_the_operators_quiet_hours_end() {
    ...
    assert_eq!(
        mute_command(&places(&["3F - Nowhere"]), &known, ONE_HOUR),
        Err(unmutable_sentence("3F - Nowhere", &known)),
    );
}

/// The refusal `mute_command` gives for a place nothing names, so a test
/// asserting it does not restate the sentence.
fn unmutable_sentence(place: &str, known: &[String]) -> String {
    match mute_command(&places(&[place]), known, Some(1)) {
        Err(said) => said,
