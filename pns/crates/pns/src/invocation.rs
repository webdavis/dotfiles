...
pub(crate) fn run() {
    ...
    // Codex's own hooks, wired into its hooks file. A MODE beside the lamps'
    // enrollment for the same reason: it is one-time wiring the apply runs, it
    // merges one file and delivers nothing.
    if first == "codex" {
        std::process::exit(codex_mode(&second_argument(&flagless)));
    }
