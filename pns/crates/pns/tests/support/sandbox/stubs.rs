...
impl Sandbox {
  ...
  *)                      exit 1 ;;
    ...
    /// Its REACH IS STATED rather than implied: the phone chain's server
    /// lookup is absolute by design (`/usr/bin/pgrep`) so no PATH can stand
    /// in front of it, and the desk idle age, the console lock and the
