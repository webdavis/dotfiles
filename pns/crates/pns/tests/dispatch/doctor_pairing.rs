...
fn the_doctor_prints_the_pairing_section_between_its_summary_and_the_decision_section() {
    ...
    let summary = lines
        .iter()
        .position(|line| *line == "3 sent, 0 failed, 5 skipped")
    ...
    assert_eq!(lines[summary + 8], HOME_UNCONFIGURED_LINE, "{printed}");
    assert_eq!(lines[summary + 9], LIGHTS_OFF_LINE, "{printed}");
    // AND THE PIN ROW AT THE FOOT OF THE LIGHTS SECTION, below the dial whose
    // refusal it reports.
    assert_eq!(lines[summary + 10], NO_CERTIFICATE_LINE, "{printed}");
...
fn a_moshi_hook_that_never_returns_does_not_park_the_doctor() {
    ...
    assert!(
        printed.contains("3 sent, 0 failed, 5 skipped"),
...
fn an_unpaired_host_exits_one_while_the_summary_still_reads_zero_failed() {
    ...
    assert!(printed.contains("3 sent, 0 failed, 5 skipped"), "{printed}");
    assert!(
        printed.contains(
