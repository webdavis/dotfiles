...
fn arming_writes_a_record_registers_a_job_and_clears_a_stale_marker_first() {
    let sandbox = Sandbox::new("remind-arms");
    ...
    assert!(
        entry.contains("id=remind:s1"),
        "one job per approval: {entry}"
    );
    assert!(
        entry.contains("marker=remind-s1"),
        "and `unless_marker` is what silences it once the answer lands: {entry}"
    );
    assert!(
        entry.contains(r#"args=["remind"]"#),
...
fn an_approval_whose_nudge_could_not_be_scheduled_leaves_no_record_behind() {
    ...
    // but any SIBLING approval's fire, and the operator's own `pns remind`, counts
    // it and cards. Bug class 19 the other way round, a stated fact the state
    // on disk contradicts.
    let sandbox = Sandbox::new("remind-registration-refused");
    sandbox.write_config(&remind_config(300));
...
fn nothing_is_armed_when_nothing_should_be() {
    ...
    for (case, config, agent) in [
        (
            "no [remind] table at all",
            remind_config(300).replace("[remind]\ndelay = \"300s\"\n", ""),
            "claude",
        ),
...
fn arming_writes_nothing_the_harness_could_read_as_a_decision() {
    ...
    let sandbox = Sandbox::new("remind-arm-stdout");
    sandbox.write_config(&remind_config(300));
    let mut command = sandbox.pns_stateful();
    ...
    assert_eq!(
        output.status.code(),
        Some(42),
        "the exit code is moshi's decision, unchanged by anything the reminder did"
    );
    assert_eq!(
