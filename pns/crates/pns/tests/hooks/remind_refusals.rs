...
fn a_record_whose_marker_appeared_is_dropped_rather_than_nudged() {
    ...
    let sandbox = Sandbox::new("remind-marker-race");
...
fn a_fire_with_the_feature_switched_off_drops_every_record_and_cards_nothing() {
    ...
    let sandbox = Sandbox::new("remind-off-drops-records");
    sandbox.write_config(&remind_config(0));
    counted_channels(&sandbox);
    write_record(&sandbox, "s1", 300, "Bash: cargo test", "wW:p21");
    ...
    assert!(
        support::stdout(&output).contains("the reminder is off; 1 waiting approval(s) dropped"),
...
fn a_record_whose_name_is_not_a_session_is_dropped_loudly_rather_than_re_read_forever() {
    ...
    let sandbox = Sandbox::new("remind-unusable-name");
    sandbox.write_config(&remind_config(300));
    counted_channels(&sandbox);
    ...
    let stray = sandbox.path("state/remind/.pending");
    std::fs::create_dir_all(stray.parent().expect("the reminder directory"))
...
#[test]
fn pns_remind_refuses_an_argument_rather_than_falling_through_to_a_fire() {
    // THE HOUSE RULE: an unknown argument never falls through to help with exit
    // 0. `pns remind <session>` is a command an operator would believe narrowed
    // the fire to one approval, and coalescing means nothing here can honour
    // it, so it is refused rather than silently ignored.
    let sandbox = Sandbox::new("remind-usage");
    sandbox.write_config(&remind_config(300));
    counted_channels(&sandbox);
    write_record(&sandbox, "s1", 300, "Bash: cargo test", "wW:p21");

    let output = remind(&sandbox)
        .arg("s1")
        .output()
...
fn a_stale_record_is_dropped_rather_than_nudged() {
    ...
    let sandbox = Sandbox::new("remind-stale");
    sandbox.write_config(&remind_config(300));
    counted_channels(&sandbox);
