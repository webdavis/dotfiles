...
fn an_unanswered_approval_is_nudged_once_through_the_ordinary_paths() {
    ...
    assert!(
        remind_marker(&sandbox, "s1").exists(),
    ...
    assert_eq!(
        remind_directory_names(&sandbox),
        Vec::<String>::new(),
...
fn a_second_fire_nudges_nothing() {
    ...
    // actually run.
    let sandbox = Sandbox::new("remind-second-fire");
    sandbox.write_config(&remind_config(300));
    counted_channels(&sandbox);
    write_record(&sandbox, "s1", 300, "Bash: cargo test", "wW:p21");
    ...
    let second = support::run(&mut remind(&sandbox));
    assert_eq!(
...
fn three_unanswered_approvals_produce_one_card_that_says_three() {
    ...
    let sandbox = Sandbox::new("remind-coalesce");
    sandbox.write_config(&remind_config(300));
    counted_channels(&sandbox);
    write_record(&sandbox, "s1", 300, "Bash: cargo test", "wW:p21");
    // 480 RATHER THAN 600, and the slack is the point twice over. The staleness
    // cap is 2 * the delay = 600, so a record armed at exactly the cap goes
...
fn fires_racing_over_one_directory_still_produce_exactly_one_card() {
    ...
    let sandbox = Sandbox::new("remind-racing-fires");
    sandbox.write_config(&remind_config(300));
    counted_channels(&sandbox);
...
fn the_daemon_really_fires_the_remind_and_really_drops_it_when_the_marker_is_there() {
    ...
    for (case, answered) in [("nobody answered", false), ("the marker is there", true)] {
        let sandbox = Sandbox::new(&format!("remind-daemon-{}", case.replace(' ', "-")));
        sandbox.write_config(&remind_config(300));
        counted_channels(&sandbox);
        write_record(&sandbox, "s1", 300, "Bash: cargo test", "wW:p21");
        if answered {
        ...
        if answered {
            ...
            let said = support::poll_until(|| {
                ...
                said.contains("remind:s1").then_some(said)
            })
            .unwrap_or_else(|| {
                panic!(
            ...
            assert!(
                said.contains("dropped `remind:s1` because its marker was already there"),
                "{case}: {said}"
            );
            ...
            assert_eq!(
                deliveries(&sandbox, "hermes"),
                0,
                "{case}: the job is dropped WITHOUT spawning a reminder process at all"
            );
        } else {
