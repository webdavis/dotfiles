...
fn an_answered_approval_is_never_nudged_by_either_clearing_signal() {
    ...
    // "the following reminder adds nothing", which is the property that matters.
    for word in ["resolved", "stop"] {
        let sandbox = Sandbox::new(&format!("remind-cleared-by-{word}"));
...
fn a_clear_landing_inside_the_fires_claim_window_still_writes_the_marker() {
    ...
    let sandbox = Sandbox::new("remind-clear-inside-claim");
    sandbox.write_config(&remind_config(300));
    counted_channels(&sandbox);
    write_record(&sandbox, "s1", 300, "Bash: cargo test", "wW:p21");
    ...
    let claim = sandbox.path("state/remind/s1.pending.claim.1");
...
#[test]
fn a_failed_turn_clears_its_approval_before_any_later_remind() {
    let sandbox = Sandbox::new("remind-cleared-by-stop-failure");
    sandbox.write_config(&remind_config(300));
    counted_channels(&sandbox);
    write_record(&sandbox, "s1", 300, "Bash: cargo test", "wW:p21");
    let output = hook_with(
    ...
    assert!(remind_marker(&sandbox, "s1").exists());
    assert_eq!(deliveries(&sandbox, "hermes"), 1);
    support::run(&mut remind(&sandbox));
    assert_eq!(
        deliveries(&sandbox, "hermes"),
        1,
        "no reminder after the failure"
    );
}
...
fn a_question_answered_inside_the_schedule_is_never_nudged() {
    ...
    for (name, payload) in [
        ...
        let sandbox = Sandbox::new(&format!("remind-answered-{}", name.replace(' ', "-")));
        sandbox.write_config(&remind_config(300));
        counted_channels(&sandbox);
