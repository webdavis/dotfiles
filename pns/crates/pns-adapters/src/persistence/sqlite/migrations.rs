...
pub(super) fn migrate(connection: &mut Connection) -> Result<(), StoreError> {
    ...
    if version < 4 {
        super::returns::schema::migrate(&transaction)?;
    }
    if version < 5 {
        super::ledger::schema::retain_request(&transaction)?;
    }
    if version < 6 {
    ...
    if version < 8 {
        super::ledger::schema::widen_failure_checks(&transaction, version)?;
    }
    if version < 9 {
        super::sessions::create(&transaction)?;
    }
    if version < 10 {
        super::session_threads::create(&transaction)?;
    }
    if version < 11 {
        super::activity::create(&transaction)?;
    }
    if version < 12 {
        super::activity::transcript_path(&transaction, version)?;
        super::recap_summaries::create(&transaction)?;
    }
    if version < 13 {
