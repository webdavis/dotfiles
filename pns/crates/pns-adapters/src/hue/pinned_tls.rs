...
//!
//! WHY THIS FILE EXISTS AT ALL. The bridge's certificate carries no
//! subjectAltName, so every modern verifier refuses to match its name against
//! the address it is reached at, whatever trust anchor is configured. The only
//! identity it carries is its own fingerprint, so that is what is pinned. ureq
//! exposes no custom-verifier hook and couples native-tls's two danger flags
//! into one, so reaching a verifier means supplying a connector through
//! `Agent::with_parts` out of ureq's `unversioned::transport` items.
//!
//! THE HANDSHAKE COMPLETES INSIDE `connect`, which is what makes a pin mismatch
//! a connection refusal rather than a mid-request surprise, and what lets the
//! observer record it before any caller has sent anything.

use super::enroll::CaptureVerifier;
...
/// The name sent and never verified.
///
/// A FIXED PLACEHOLDER, because rustls needs a `ServerName` to build a
/// connection and the certificate carries none that could ever match. Server
/// name indication is turned off below, so nothing reaches the bridge.
