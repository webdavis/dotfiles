use super::FileRemindRecords;
use crate::marker_files::marker_path;
use crate::remind_records::{
    claim_path, claim_record, parse, record_entries, record_path, remind_dir,
};
use pns_application::Claimed;
use pns_domain::remind::{Dropped, marker_name, session_of};

impl FileRemindRecords {
    pub(super) fn claimed_records(&self) -> Vec<Claimed> {
        let mut held = Vec::new();
        for record in record_entries(&remind_dir(&self.state)) {
            ...
            let Some(session) = record
                ...
                eprintln!(
                    "pns remind: {} is not named for a session this can act on; it is dropped",
                    record.display()
                );
    ...
    pub(super) fn retire_claim(&self, session_id: &str, reason: Option<Dropped>) {
        ...
        if reason == Some(Dropped::Unreadable) {
            eprintln!(
                "pns remind: {} is not a record this can read; it is dropped",
                record.display()
            );
        }
        ...
        {
            eprintln!(
                "pns remind: the working file {} could not be removed ({error}); it is left behind",
                claim.display()
            );
        }
    }
