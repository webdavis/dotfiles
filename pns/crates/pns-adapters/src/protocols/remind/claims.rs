...
use std::path::Path;
/// Every file in the reminder directory that could be a record, sorted so a fire is
/// deterministic.
///
...
pub fn release_fire(fire: &Path) {
    if let Err(error) = std::fs::remove_file(fire) {
        eprintln!(
            "pns remind: the fire claim {} could not be given up ({error}); the next fire waits it out",
            fire.display()
        );
    }
}
