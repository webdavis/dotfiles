use pns_domain::remind::{FIRE_STALE_SECS, RECORD_SUFFIX, Record, usable};
...
pub fn remind_dir(state_dir: &std::path::Path) -> std::path::PathBuf {
    state_dir.join("remind")
}
...
pub fn record_path(state_dir: &std::path::Path, session_id: &str) -> Option<std::path::PathBuf> {
    usable(session_id).map(|id| remind_dir(state_dir).join(format!("{id}{RECORD_SUFFIX}")))
}
...
mod records;
pub use records::FileRemindRecords;
