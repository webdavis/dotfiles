...
use super::{Record, claim_path, parse, record_path, render};
use pns_domain::remind::{MAX_SESSION_ID_CHARS, job_id, marker_name, session_of};
use std::path::Path;
...
fn an_ordinary_session_id_names_a_record_a_marker_a_job_and_a_claim() {
    ...
    assert_eq!(
        ...
        Some(Path::new("/s/remind/abc-123.pending").to_path_buf())
    );
    assert_eq!(marker_name("abc-123"), Some("remind-abc-123".to_string()));
    assert_eq!(job_id("abc-123"), Some("remind:abc-123".to_string()));
...
fn a_session_id_that_cannot_be_a_filename_names_nothing_at_all() {
    ...
    // THE CEILING IS EXACTLY WHERE THE DAEMON STOPS. `remind:<id>` is the job
    // id and `remind-<id>` the marker, and both are refused past the daemon's
...
fn two_ids_that_differ_only_after_a_dot_claim_two_different_names() {
    ...
    assert_eq!(
        first,
        Path::new("/s/remind/a.b.pending.claim.7").to_path_buf(),
