...
fn a_mistyped_key_inside_a_plugin_table_is_refused_naming_the_table_and_the_key() {
    ...
    // miss was a destination that quietly never worked: `keys` for `key` is
    // a bridge nothing can authenticate against, and `tokens` for `token` is
    // a phone card that silently never leaves the machine.
    for (table, mistyped, near) in [
        ("plugins.log", "key", "keys"),
        ("plugins.lights", "keys", "key"),
