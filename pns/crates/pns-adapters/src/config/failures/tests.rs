...
fn a_file_with_no_table_serves_the_page_at_the_documented_port() {
    ...
    assert!(defaults.page_enabled);
    assert_eq!(defaults.page_port, 8646);
}
...
fn serving_can_be_switched_off_without_disturbing_the_port() {
    ...
    assert!(!off.page_enabled);
    assert_eq!(off.page_port, 9000);
}
...
fn a_privileged_port_is_refused_by_name() {
    ...
    assert!(error.contains("`page_port`"), "{error}");
...
fn a_value_of_the_wrong_shape_names_the_key_and_its_type() {
    let error = refusal("[failures]\npage_enabled = \"yes\"\n");
    assert!(error.contains("`page_enabled`"), "{error}");
...
/// The spellings these two keys replaced. Each is refused by name, and the
