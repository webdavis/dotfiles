...
/// either of them: `[lights.blocked] lease_expiry` and `[remind] delay`
...
///
/// THE BACKSTOP DARKENS AN UNANSWERED WAIT'S LAMP at `lease_expiry` and
...
/// `lease_expiry` has a floor of 60, so the comparison below is already
/// false for an off reminder; and `DEFAULT_BLOCKED_LEASE_EXPIRY_SECS` (16
...
pub(super) fn backstop_outlasts_the_reminder(config: &Config) -> Result<(), ConfigError> {
    ...
    let give_up = lights.blocked.lease_expiry_secs;
    if give_up < config.remind_delay_secs {
        return Err(ConfigError::Invalid(format!(
            "`lights.blocked` key `lease_expiry` is {give_up}, below `remind` key \
