mod fixtures {
    ...
    impl pns_adapters::Bridge for ScriptedBridge {
    ...
    impl pns_adapters::Bridge for SlowBridge<'_> {
        fn get(&self, path: &str) -> Option<String> {
    ...
    pub(crate) fn noon(muted: &pns_domain::lamps::Muting) -> pns_domain::lamps::Reading<'_> {
        pns_domain::lamps::Reading {
    ...
    pub(crate) fn nothing_muted() -> pns_domain::lamps::Muting {
        pns_domain::lamps::Muting::Places(Vec::new())
    }

    /// One place the operator quieted by hand.
    pub(crate) fn quieted(place: &str) -> pns_domain::lamps::Muting {
        pns_domain::lamps::Muting::Places(vec![place.to_string()])
    }

    pub(crate) fn read_held(
        state: &std::path::Path,
    ) -> Option<Vec<pns_domain::lights::phase::HeldEntry>> {
    ...
    impl pns_adapters::Bridge for TwoRoomBridge {
        fn get(&self, path: &str) -> Option<String> {
            Some(
                match path {
                    "light" => {
