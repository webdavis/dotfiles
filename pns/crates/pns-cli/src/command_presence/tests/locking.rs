mod tests {
    ...
    impl pns_adapters::Bridge for ParkedBridge {
    ...
    fn a_poll_already_running_stands_a_second_one_down_rather_than_racing_it() {
        ...
        assert_eq!(
            std::fs::read_to_string(state.join(pns_adapters::PRESENCE_STATE_FILE))
    ...
    fn the_lock_a_killed_poller_left_behind_does_not_stand_the_next_poll_down() {
        ...
        std::fs::write(state.join(pns_adapters::PRESENCE_LOCK_FILE), b"").expect("the leavings");
