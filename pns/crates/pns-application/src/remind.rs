//! The reminder: one card about every approval still waiting, at most one run at a
//! time.
//!
...
use crate::{RaiseNotification, RemindRecords};
use pns_domain::EventArgs;
use pns_domain::remind::{self, Fate, Record};

/// What a reminder run answered, as an exit code and what it did.
...
/// The ports one reminder fire works over.
pub struct RunRemind<'a, R, N> {
    pub records: &'a R,
    pub notifier: &'a N,
}

impl<R: RemindRecords, N: RaiseNotification> RunRemind<'_, R, N> {
    ...
    pub fn run(&self, now: u64, after_secs: u64, mut warn: impl FnMut(&str)) -> Outcome {
        ...
        for claimed in self.records.claim_due(now) {
            match remind::fate(claimed.record.as_ref(), claimed.answered, now, after_secs) {
                Fate::Count => match claimed.record {
                    ...
                    None => self
                        .records
                        .drop_claim(&claimed.session_id, Some(remind::Dropped::Unreadable)),
                },
        ...
        for (session_id, _) in &waiting {
            if let Err(error) = self.records.mark_answered(session_id) {
                warn(&format!(
                    "pns remind: an answered marker could not be written ({error})"
                ));
            }
        }
