...
/// tool batch or ends a turn, rather than one per session that armed a reminder.
...
pub fn clear_remind<R: RemindRecords>(records: &R, session_id: &str, mut warn: impl FnMut(&str)) {
    if pns_domain::remind::usable(session_id).is_none() {
        return;
    }
