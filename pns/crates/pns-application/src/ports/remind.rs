...
use pns_domain::remind::{Dropped, Record as RemindRecord};

/// This session's remind: the schedule that nudges about a wait nobody answered.
///
...
/// answered. Which agents are reminded at all, and after how long, is the adapter's
/// read of the config. Statements: S074, S237.
pub trait RemindSchedule {
    fn arm(&self, session_id: &str, event: &EventArgs);
}

/// The reminder's own records: one per session waiting on an approval.
///
...
/// claims and reports; the use case applies `remind::fate` to the parsed record
/// and answered marker. Returning only survivors would put that policy in
/// the adapter.
///
/// `src/command_remind.rs` owns fire claims and disabled cleanup;
/// `src/remind_schedule_runtime.rs` owns arming, rollback and answered markers.
/// Statements: S182, S236, S237, S241.
pub trait RemindRecords {
    ...
    fn publish(&self, session_id: &str, record: &RemindRecord) -> Result<(), String>;
...
pub struct Claimed {
    ...
    /// `None` where nothing parsed out of the file, which `remind::fate` reads as
    /// a record to drop rather than one to count.
    pub record: Option<RemindRecord>,
