    ...
    D: NotificationDestination,
{
    pub(super) fn attempt(
        ...
        claim: Option<(&L::Claim, LeaseWindow, pns_domain::retry::RetryBackoff)>,
        destination: &str,
        clock: &(impl Clock + Sync),
        notice: &(impl Fn(&str) + Sync),
    ) -> Delivery {
        let record = |request: &DeliveryRequest<'_>, delivery: &Delivery| {
            if let Some((claim, lease, backoff)) = claim {
                let at = clock.now_secs().unwrap_or(lease.now);
                if let Err(error) = self.ledger.record(claim, delivery, at, backoff) {
                    notice(&format!(
