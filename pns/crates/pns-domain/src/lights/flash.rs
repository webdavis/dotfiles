...
/// same reason: `github` is one routable word carrying TWO colours, exactly as
/// `unread` is, so a config cannot route a GitHub pass without its failure and
/// there is no spelling for trying. The flavour is the EVENT's, never the
/// config's.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Flash {
    /// Any behaviour whose colour is fixed, which is every word but `github`.
    /// `Behaviour::Github` spelled here renders nothing, the same fail-dark
...
impl Flash {
    /// The ROUTABLE word this pulse is carried by, which is what a `shows`
    /// list is matched against.
    pub fn behaviour(self) -> crate::lamps::config::Behaviour {
        match self {
            Flash::Word(word) => word,
            Flash::GithubPass | Flash::GithubFail => crate::lamps::config::Behaviour::Github,
        }
    }
}
