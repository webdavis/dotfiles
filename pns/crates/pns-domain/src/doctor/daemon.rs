...
/// The reminder's own line in the doctor's tail run: what the schedule is, said in
/// the unit the card will say it in.
///
/// IT REPORTS THE CONFIG ONLY AND DOES NOT GRADE THE DAEMON. A reminder with a dead
...
///
/// IT DOES NOT MOVE THE EXIT CODE, for `focus_line`'s reason: a reminder being off
...
pub fn remind_line(after_secs: u64) -> String {
    match after_secs {
        0 => format!("{PREFIX}the reminder is off (no `[remind] delay`)"),
        seconds => format!(
...
#[cfg(test)]
mod remind_tests {
    use super::remind_line;

    #[test]
    fn the_remind_line_names_the_schedule_or_says_the_feature_is_off() {
        assert_eq!(
            remind_line(0),
            "pns doctor: the reminder is off (no `[remind] delay`)"
        );
        assert_eq!(
            remind_line(300),
