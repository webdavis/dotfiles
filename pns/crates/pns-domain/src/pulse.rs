...
/// The `checks` lamp's PASS colour: the purple the 2026-05-18 research chose
...
/// survivable only on a lamp of its own: `checks` belongs on a lamp whose
/// `behaviours` list names nothing else, where the only two colours it ever shows
/// are this and the orange below, 0.399 apart.
pub const CHECKS_PASS_COLOR: PulseColor = PulseColor {
    x: 0.2725,
    y: 0.1283,
};

/// The `checks` lamp's FAIL colour: orange, from the same research.
///
...
/// different orange.
pub const CHECKS_FAIL_COLOR: PulseColor = PulseColor {
    x: 0.5562,
    y: 0.4084,
};
