...
1. **A real card on the operator's phone.** The `mobile` leg posts the event to the moshi webhook
   ...
   set by `[plugins.mobile] url`, else `PNS_MOSHI_URL`).
...
- Thresholds: the roster is exactly six registrations (`src/registry.rs:ROSTER`) in the order `home_presence`,
  `presence`, `mobile`, `banner`, `hermes`, `lights`. The two sensors lead; registration order is
...
- Success: `[plugins.home_presence] enabled = true` gives
  ...
  `[plugins.lights] enabled = true` gives `Pulse`; `mobile`, `banner` and `hermes` each give `Send`.
...
- Failure sources: `src/main.rs:disabled_backend_warnings` emits one line per switched-off `home_presence` or
  ...
  `pns: config error (<detail>); running the core plugins (mobile, banner)` for one that could not
...
- Success:
  `tests/dispatch.rs:the_doctor_sends_its_labelled_payload_to_every_enabled_channel_and_reports_each_one`
  asserts, for each of `mobile`, `banner` and `hermes`, that the delivered event carries
...
- Success:
  ...
  `PNS_FORCE_PHONE=1`, and asserts every one of `mobile`, `banner` and `hermes` still fired, with
  exit 0.
