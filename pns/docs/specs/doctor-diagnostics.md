...
read-only sections (moshi pairing, macOS Focus, the daemon, the reminder, the lamps, the decision ring, and
...
1. **A real macOS banner.** The `banner` leg spawns `terminal-notifier` with the doctor's title and
...
1. **A real card on the operator's phone.** The `phone` leg posts the event to the moshi webhook
   (`https://api.getmoshi.app/api/webhook` by default, `src/channels/moshi.rs:DEFAULT_MOSHI_URL`,
   set by `[plugins.phone] url`, else `PNS_MOSHI_URL`).
...
1. **Real lamp writes.** The `lights` check is a `Pulse`, and `src/main.rs:pulse_outcome` calls
   ...
   flash. It runs inside the **quiet window** deliberately, pinned
   by `tests/dispatch/doctor_delivery.rs:the_doctor_reaches_the_bridge_whatever_the_hour`.
1. **Executable channels are spawned.** When `[paths] channels_dir` or `PNS_CHANNELS_DIR` is set, native plugins lose and the
...
The suite's own pattern is `tests/dispatch.rs:doctor_command`, which points `PNS_STATE_DIR` (the variable `[paths] state_dir` now leads) into a
...
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `home_presence`              | Nothing. It is a `PluginKind::Sensor`, and no leg can route to a sensor (`src/doctor.rs:kind_of`).                                                                                                                                                                                                                                               | **None.**                                                                                           | Not applicable.                                                                                                                                                                                                                    | `Skipped` only.                                                                                                                                                                                                     | `home_presence: skipped, a sensor and never a delivery destination`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `src/doctor.rs:a_selected_sensor_is_a_skip_because_no_leg_can_ever_reach_one`, `src/doctor.rs:a_line_names_its_plugin_and_its_outcome_and_a_failure_quotes_the_channel`, `tests/dispatch.rs:the_doctor_sends_its_labelled_payload_to_every_enabled_channel_and_reports_each_one`                                                                                                                                                                            |
| `presence`                  | Nothing sent. It is a `PluginKind::Sensor` too, but its READING is printed in place of the skip (`src/doctor.rs:kind_of`): the state file's one line read through `src/system.rs:readable_state_file`, parsed by `src/presence_file.rs:parse_presence_line` and judged by `src/presence.rs:classify` against the probe set's own memoized clock. | **None.** One file read, no subprocess and no bridge call.                                          | Not applicable. The read refuses a non-regular file rather than opening it and caps at `src/presence_file.rs:READ_MAX`, 4,096 bytes.                                                                                               | `Presence` only, carrying one of seven states: a room, nowhere, and the five ways of not knowing. Never `Sent` and never `Failed`; the verdict is `Skipped`, so a reading moves the exit code in neither direction. | `presence: <room> (<n>s ago)`, `presence: nowhere (poll <n>s ago)`, `presence: unknown (no reading)`, `presence: unknown (the clock could not be read)`, `presence: unknown (stale, poll <n>s old)`, `presence: unknown (future epoch)`, `presence: unknown (the reported room is not one this config watches)`. The room is the bridge's own text and crosses `src/doctor.rs:printable` first; a name nothing survives prints as `a room whose name will not print`. With a narrowing already recorded the line gains `; last narrowed to <room>` or `; last narrowed nothing (<reason>)`, read by KEY off the last JSON object in the `presence-decisions` ring (`src/main.rs:last_narrowing`, `src/presence_journal.rs:last`) and crossing the same filter. The two are told apart structurally, by whether the record names a room, so a room literally called `nothing (away)` cannot read as a fallback. | `src/doctor.rs:a_known_room_is_named_with_the_age_of_its_motion_edge`, `src/doctor.rs:a_fresh_poll_that_found_nobody_says_nowhere_rather_than_unknown`, `src/doctor.rs:every_way_of_not_knowing_says_which_way_it_is`, `src/doctor.rs:a_room_name_the_bridge_chose_is_filtered_before_it_reaches_the_terminal`, `src/doctor.rs:a_reading_is_never_counted_as_a_send_however_good_it_is`                                                                     |
| `phone`                    | One real push of the doctor event to the moshi webhook.                                                                                                                                                                                                                                                                                          | **YES: a card on the operator's phone.**                                                            | `src/channels/moshi.rs:POST_DEADLINE`, 10 seconds.                                                                                                                                                                                 | `Sent`, `SentUnreported`, `Failed`, `Skipped`.                                                                                                                                                                      | `phone: sent, <what the channel said>` / `phone: sent, this channel reports no outcome` / `phone: FAILED, push SKIPPED -- no moshi token in the config ([plugins.phone] token); nothing was sent` / `phone: skipped, <reason>`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `tests/dispatch.rs:the_doctor_sends_its_labelled_payload_to_every_enabled_channel_and_reports_each_one`, `tests/dispatch.rs:a_failure_on_the_first_channel_costs_no_later_leg_its_turn_and_still_exits_one`, `tests/dispatch.rs:the_doctor_names_the_type_when_the_type_is_the_fault_and_never_the_token`                                                                                                                                                   |
| `banner`              | One real banner through `terminal-notifier`.                                                                                                                                                                                                                                                                                                     | **YES: a notification on the operator's screen.**                                                   | `src/system.rs:PROBE_DEADLINE`, 5 seconds (the runner's bounded spawn).                                                                                                                                                            | `Sent`, `SentUnreported`, `Failed`, `Skipped`.                                                                                                                                                                      | `banner: sent, posted the banner` / `banner: FAILED, banner FAILED (terminal-notifier did not run)` / `banner: sent, this channel reports no outcome`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `tests/dispatch.rs:a_failure_on_the_first_channel_costs_no_later_leg_its_turn_and_still_exits_one` (asserts `banner: sent, posted the banner` and that the notifier really received its payload)                                                                                                                                                                                                                                                      |
| `hermes`                    | One signed HTTP post to the gateway route.                                                                                                                                                                                                                                                                                                       | **YES: an entry in the operator's durable log route.**                                              | `src/channels/hermes.rs:remote_deadline`, 5 seconds by default (`DEFAULT_REMOTE_DEADLINE_SECS`), set by `[delivery] remote_deadline`, capped at `MAX_SYNC_DEADLINE_SECS` (86,400 seconds); the value `0` means no deadline at all. | `Sent`, `SentUnreported`, `Failed`, `Skipped`.                                                                                                                                                                      | `hermes: sent, <the post's own sentence>` / `hermes: FAILED, post SKIPPED -- no hermes key for the <route> route ([plugins.log.keys] <route>); nothing was sent`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | `tests/dispatch.rs:a_failure_on_the_first_channel_costs_no_later_leg_its_turn_and_still_exits_one`, `src/doctor.rs:a_line_names_its_plugin_and_its_outcome_and_a_failure_quotes_the_channel`                                                                                                                                                                                                                                                                |
| `lights` (pulse)             | One `PUT` per addressed room, `Behaviour::Done`. No event dispatches to hue, so it is checked as a `Pulse` rather than a `Send` (`src/doctor.rs:kind_of`).                                                                                                                                                                                       | **YES: the operator's lamps flash**, quiet window included.                         | `src/channels/hue.rs:BRIDGE_DEADLINE`, 10 seconds per call (one `GET` of the room listing plus one `PUT` per room).                                                                                                                | `Signalled(n)` for n above zero, `Signalled(0)` (graded a failure), `Failed` for a config that resolves to no bridge or for a panic, `Skipped`.                                                                     | `lights: signalled 1 room (watch for the flash; the bridge acknowledges no write)` / `lights: signalled 2 rooms (watch for the flash; the bridge acknowledges no write)` / `lights: FAILED, signalled no rooms (no room listing from the bridge, or no configured room name matched)` / `lights: FAILED, pulse SKIPPED -- no hue bridge and key in the config ([plugins.lights] bridge, key); nothing was signalled` / `lights: FAILED, the pulse PANICKED; no room was signalled`                                                                                                                                                                                                                                                                                                                                                                                                                                               | `src/doctor.rs:the_pulse_line_claims_neither_a_flash_nor_a_cause_it_cannot_know`, `tests/dispatch.rs:a_pulse_with_no_bridge_to_dial_names_the_settings_rather_than_the_rooms`, `tests/dispatch.rs:a_pulse_the_bridge_answered_nothing_for_still_names_both_causes_it_cannot_choose_between`, `tests/dispatch/doctor_delivery.rs:the_doctor_reaches_the_bridge_whatever_the_hour`                                                                               |
| moshi pairing, local leg    | `moshi-hook status --json`, which carries the pairing fact pns grades.                                                                                                                                                                                                                                                                           | Spawns another program. No network today (measured local-only).                                     | `src/main.rs:MOSHI_JSON_DEADLINE`, 5 seconds, overridable with `PNS_MOSHI_JSON_DEADLINE`.                                                                                                                                       | `Paired`, `Unpaired`, `Unreadable`, `NoAnswer`.                                                                                                                                                                     | See behavior 15 for all four sentences.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `tests/dispatch.rs:the_doctor_runs_moshi_hook_exactly_twice_and_never_probes`, `tests/dispatch.rs:a_moshi_hook_that_never_returns_does_not_park_the_doctor`                                                                                                                                                                                                                                                                                                 |
| moshi pairing, relay leg    | Plain `moshi-hook status`, the only shape carrying a server verdict.                                                                                                                                                                                                                                                                             | **YES: a call to the moshi API.** This is the only network call the doctor makes for its own sake.  | `src/main.rs:MOSHI_STATUS_DEADLINE`, 8 seconds, overridable with `PNS_MOSHI_STATUS_DEADLINE`. It must exceed moshi's own internal timeout, measured at about 5.1 seconds against an unroutable base URL.                        | A relayed sentence, or nothing.                                                                                                                                                                                     | `pns doctor: moshi says: <moshi's own sentence>`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `tests/dispatch.rs:the_doctor_runs_moshi_hook_exactly_twice_and_never_probes`, `src/doctor.rs:the_server_line_is_relayed_as_moshis_own_words_with_the_label_removed`, `src/doctor.rs:only_a_server_line_at_column_zero_is_relayed`                                                                                                                                                                                                                          |
| macOS Focus                 | Reads `~/Library/DoNotDisturb/DB/Assertions.json` and `ModeConfigurations.json` when, and only when, `[focus] modes` names something.                                                                                                                                                                                                          | **None.** Read-only, and nothing is opened at all when the list is empty (`src/main.rs:focus_now`). | Not applicable. Bounded by size at `src/main.rs:RING_READ_MAX`, 262,144 bytes.                                                                                                                                                     | Off, on, quiet, unreadable, absent, plus a catalog clause.                                                                                                                                                          | See behavior 19.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `tests/dispatch.rs:the_doctor_tells_the_truth_about_a_named_focus_in_every_state`, `tests/dispatch.rs:a_mode_catalog_the_doctor_cannot_read_is_said_and_never_reported_as_health`                                                                                                                                                                                                                                                                           |
...
| remind                         | The config's `[remind] delay` alone.                                                                                                                                                                                                                                                                                                           | **None.** No file is read for it beyond the one config read the doctor already took.                | Not applicable.                                                                                                                                                                                                                    | Two states.                                                                                                                                                                                                         | `` pns doctor: the reminder is off (no `[remind] delay`) `` / `pns doctor: an unanswered approval is carded again after <duration>`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `src/doctor.rs:the_remind_line_names_the_schedule_or_says_the_feature_is_off`                                                                                                                                                                                                                                                                                                                                                                                  |
...
- Success: with nothing enabled at all, the returned plugin names still equal `registry.names()`, pinned
  ...
  asserts all six lines are still printed.
...
- Thresholds: the roster is exactly six registrations (`src/registry.rs:ROSTER`) in the order `home_presence`,
  `presence`, `phone`, `banner`, `hermes`, `lights`. The two sensors lead; registration order is
  delivery order for the channels behind them.
...
Then not-selected is asked FIRST, then the room sensor is a `Presence` reading, then any other sensor is
`Skipped`, then a channel no event dispatches to is a `Pulse`, and every other channel is a `Send`.

- Success: `[plugins.home_presence] enabled = true` gives
  `Skipped("a sensor and never a delivery destination")`; `[plugins.presence] enabled = true` gives
  `Presence`, because a reading is the one thing about that sensor an operator cannot see any other way;
  `[plugins.lights] enabled = true` gives `Pulse`; `phone`, `banner` and `hermes` each give `Send`.
  Pinned by `src/doctor.rs:a_selected_sensor_is_a_skip_because_no_leg_can_ever_reach_one`,
  `src/doctor.rs:the_selected_room_sensor_is_a_reading_rather_than_the_sensor_skip`,
...
- Process ownership and cleanup: Not applicable.
- Compatibility contract: `lights` is registered with `event_dispatched: false` (`src/registry.rs:ROSTER`),
...
- Success: `ConfigState::Read` gives `not enabled in the config`; `ConfigState::Absent` gives
  ...
  asserts the absent wording for `home_presence`, `hermes` and `lights`.
...
When the config names an unknown or duplicate plugin, switches a plugin on without the one it borrows a
credential from, or cannot be read, or holds a switched-off table whose `type` names no compiled-in
backend, or writes a `[plugins.presence]` table this cannot read
...
- Success:
  ...
  asserts the doctor's standard error names `[plugins.home_presence]` and `switched off`, and that the same
  config produces no such line on the event path.
- Failure sources: `src/main.rs:disabled_backend_warnings` emits one line per switched-off `home_presence` or
  `phone` table whose type is unrecognized; `src/registry.rs:select_plugins` returns
  `pns: config error (unknown plugin `<name>`); running every built-in plugin` for a parsed config with a
  bad table name,
  `` pns: config error (`presence` is enabled and needs `lights`, which is not); running every built-in plugin ``
  for a borrowed credential nobody switched on, and
  `pns: config error (<detail>); running the core plugins (phone, banner)` for one that could not
  be read. `src/main.rs` prints `pns: config error (<detail>); the room sensor is unread` for a
  `[plugins.presence]` table `src/config.rs:parse_presence` refused, and the census still prints its
  `presence:` line, as `unknown (no reading)`.
...
- Idempotency and duplicates: one fault, one line.
  `tests/dispatch.rs:a_phone_table_naming_no_compiled_in_backend_pushes_no_card_through_either_seam`
...
- Privacy: the type value is quoted (`"pushover"`); the token is never quoted. Pinned by
  ...
  asserts the phone line contains `"pushover"` and `type` and does NOT contain `token`.
- Process ownership and cleanup: Not applicable.
...
- Success:
  `tests/dispatch.rs:the_doctor_sends_its_labelled_payload_to_every_enabled_channel_and_reports_each_one`
  asserts, for each of `phone`, `banner` and `hermes`, that the delivered event carries
...
- Success:
  ...
  `PNS_FORCE_PHONE=1`, and asserts every one of `phone`, `banner` and `hermes` still fired, with
  exit 0.
...
- Success:
  `tests/dispatch.rs:a_failure_on_the_first_channel_costs_no_later_leg_its_turn_and_still_exits_one`
  fails the FIRST dispatched channel (`phone`, no token), asserts the banner behind it delivered AND was
...
When the doctor reaches the `lights` check
...
- Failure sources: a `[plugins.lights]` table with a `bridge` and no `key` (or vice versa) resolves to
...
- Compatibility contract, verbatim (`src/main.rs:NO_HUE_BRIDGE_LINE`, rendered through
  `src/doctor.rs:line`):
  `lights: FAILED, pulse SKIPPED -- no hue bridge and key in the config ([plugins.lights] bridge, key); nothing was signalled`.
...
- Timeout and cancellation: `src/system.rs:run_bounded` runs the read on a thread, polls the child with a
  ...
  turn (with `PNS_MOSHI_STATUS_DEADLINE=200` and `PNS_MOSHI_JSON_DEADLINE=200`) and asserts the
...
- Success: an empty `[focus] modes` list prints the OFF sentence without opening any file (the census
...
- Thresholds: the two store files are read through `readable_state_file` at `RING_READ_MAX`, 262,144
  bytes; the live store is 6 KiB. A store past that ceiling reads as `FileTooLarge`, which is the
  unreadable sentence.

- Required side effects: none. Read-only, and with no `[focus] modes` list the files are never opened,
...
- Timeout and cancellation: a named pipe at either path is refused by `readable_state_file`'s
  regular-file check rather than opened, so it cannot park the doctor.
...
### 21. The reminder line, in two states

Given the config's `[remind] delay` is the whole input

When `src/doctor.rs:remind_line` renders it

Then zero prints the off sentence and anything else prints the schedule, in the same unit the card uses.

- Success: `src/doctor.rs:the_remind_line_names_the_schedule_or_says_the_feature_is_off` asserts all three:
  0 gives the off sentence, 300 gives `after 5m`, and 30 gives `after 30s`. The unit comes from
  `src/remind.rs:waited`: under 60 seconds prints seconds, under 3600 prints whole minutes, otherwise whole
  hours.
- Failure sources: none. The value is range-bound at parse time and this function is total over `u64`.
- Fail direction: Not applicable.
- Thresholds: `src/main.rs:REMIND_OFF` is 0, and it is also the config default (`src/config.rs`,
  `remind_delay_secs: REMIND_OFF`). Two states and not three: no `[remind]` table and no `delay` key are the
...
- Forbidden side effects: it reports the config only and does NOT grade the daemon. The doc says why: a
  remind with a dead daemon never fires, but the daemon line one row above already says the daemon is not
  running, from the heartbeat, and two lines deriving one fact is how they drift apart.
- Timeout and cancellation: Not applicable, pure.
- Idempotency and duplicates: pure.
- Privacy: a duration only.
- Process ownership and cleanup: Not applicable.
- Compatibility contract, verbatim: `` pns doctor: the reminder is off (no `[remind] delay`) `` and
...
- Failure sources: `Pairing::Unpaired` alone moves it, pinned by
  ...
  while the summary still reads `pns doctor: 3 sent, 0 failed, 3 skipped`.
...
- Forbidden side effects: the Focus line, the daemon line, the reminder line, the lights section, the decision
...
- Compatibility contract: `src/doctor.rs:exit_code`'s doc names the rule the two hardest cases share: "A
  ...
  an unregistered host means every card is going nowhere while the census reports the phone channel
...
- Success: the order is the opening, one line per check, the summary, the pairing line (plus the relayed
  line when there is one), the Focus line, the daemon line, the reminder line, the lights section, the
  decision ring section, the journal count, and any retained import diagnostics. Pinned end to end by
  `tests/dispatch.rs:the_doctor_prints_the_pairing_section_between_its_summary_and_the_decision_section`,
...
### 27. Doctor records no event of its own

Given retained internal history, when doctor reads it, then its first access may create the private
database and commit the one-time import described in decision 0014. This replaces the historical
file-only assertion that the whole state directory stays byte-identical. Original legacy files remain
unchanged, and repeated reads preserve the imported logical records.

Doctor never appends a decision, missed notification or activity event for its own test send, and never
claims a return batch. The existing tests `the_doctor_records_no_decision_of_its_own`,
`the_doctor_leaves_the_journal_exactly_as_it_found_it`,
`the_doctor_reaches_every_channel_through_a_mute_a_desk_and_both_phone_overrides`, and
`the_pairing_check_records_nothing_of_its_own` inspect the actual stored rows as well as retained legacy
bytes. Physical notifications remain the doctor's intentional effect.

### 28. Import diagnostics do not change the delivery grade

Given unreadable imported records, when doctor completes its history sections, then it names each
retained failure after the journal count. Each line identifies a record family and a non-secret reason,
and states that the legacy file remains. An unavailable import check is reported as unknown. Neither case
changes the existing delivery-health exit code, appends an event or discloses imported content.
