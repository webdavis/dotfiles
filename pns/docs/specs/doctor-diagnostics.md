...
- Thresholds: the roster is exactly six registrations (`src/registry.rs:ROSTER`) in the order `router`,
  `presence`, `mobile`, `macos-banner`, `hermes`, `hue`. The two sensors lead; registration order is
...
- Success: `[plugins.router] enabled = true` gives
  ...
  `[plugins.hue] enabled = true` gives `Pulse`; `mobile`, `macos-banner` and `hermes` each give `Send`.
...
- Failure sources: `src/main.rs:disabled_backend_warnings` emits one line per switched-off `router` or
  ...
  bad table name,
  `` pns: config error (`presence` is enabled and needs `hue`, which is not); running every built-in plugin ``
  for a borrowed credential nobody switched on, and
  `pns: config error (<detail>); running the core plugins (mobile, macos-banner)` for one that could not
