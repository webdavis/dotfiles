...
settles what the router IS in this crate: `[plugins.home_presence]` is registered as a SENSOR, an input, never a
...
| No `[plugins.home_presence]` table                                                                     | No reading is taken                                                | `home: not configured (no [plugins.home_presence] table)`                                                                                                                                                 | Refuse to read, exit 0                                                                                                                                                                        |
| `[plugins.home_presence] enabled = false`                                                              | No reading is taken                                                | `home: [plugins.home_presence] is present but enabled = false`                                                                                                                                            | Refuse to read, and told apart from the missing table on purpose, because the two are different edits (`src/home.rs:enabled_router_table`)                                                    |
| `type` absent or empty                                                                          | No reading is taken                                                | `home: no type in [plugins.home_presence] (the only type is "unifi")`                                                                                                                                     | Refuse to read                                                                                                                                                                                |
| `type` names no compiled-in backend                                                             | No reading is taken                                                | `home: [plugins.home_presence] has type "asus", which no compiled-in backend answers (the only type is "unifi")`                                                                                          | Refuse to read, quoting what was written                                                                                                                                                      |
| `router_url` missing, empty, or not a string                                                    | No reading is taken                                                | `home: the [plugins.home_presence] table is present but router_url is missing, empty, or not a string`                                                                                                    | Refuse to read                                                                                                                                                                                |
| All three device keys absent                                                                    | No reading is taken                                                | `home: no device to look for in [plugins.home_presence] (set at least one of device_mac, device_hostname, device_ipv4)`                                                                                   | Refuse to read. A probe with nothing to look for would read NotHome forever (`src/home.rs:device_identity`)                                                                                   |
| A device key present but empty, the wrong TOML type, or unreadable as its shape                 | No reading is taken                                                | `home: device_ipv4 = "192.168.1" in [plugins.home_presence] is not an IPv4 address (a dotted quad, e.g. "192.168.1.169")`, and the sibling lines for the media access control address and the client name | Refuse by NAME. A blank key read as absent would be a silent typo the output could not show (`src/home.rs:read_device_key`)                                                                   |
| `api_key` absent, empty, or not a string                                                        | No reading is taken                                                | `home: no api_key in the [plugins.home_presence] table (the probe is not set up)`                                                                                                                         | Refuse to read; every way the key is missing is one line (`src/home.rs:router_api_key`)                                                                                                       |
...
When the config enables `[plugins.home_presence]` and an event is dispatched
...
- Success: `src/registry.rs:ROSTER` declares `name: "router"` with `kind: PluginKind::Sensor`, first in
  ...
  prints `home_presence: skipped, a sensor and never a delivery destination` (`src/doctor.rs:kind_of`,
  `src/doctor.rs:A_SENSOR`). Pinned by `tests/dispatch.rs:the_binarys_own_roster_knows_the_router_sensor`
  (a recording stub is installed under the name `home_presence` so a rogue leg would leave a trace, and it does
...
- Required side effects: the name `home_presence` occupies the one plugin namespace, so `[plugins.home_presence]` is a
  known table and the roster fallback is not triggered by it.
- Forbidden side effects: no event may be delivered to `home_presence`, under any flag, fallback, or later kind.
...
- Compatibility contract: the config table is spelled `[plugins.home_presence]`. The retired top-level `[home]`
...
Given a `[plugins.home_presence]` table
