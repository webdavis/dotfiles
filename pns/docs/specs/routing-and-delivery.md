...
- **Compatibility contract:** `PostOutcome::NoStatus` and `PostOutcome::NoResponse` are two different
  ...
  bash's missing `-f`. The default gateway is `http://127.0.0.1:8644/webhooks/pns-events`
  (`:the_default_url_is_the_local_gateway_route`).

### 16. A route is a NAME, and it swaps the gateway's final path segment

Given `--channel <route>` on an event, or a route named in config for the stale alert

When `hermes_url_for` runs

Then `[plugins.hermes] url`, and `PNS_HERMES_URL` after it, win outright if set non-empty; else an empty route posts to the default; else
...
- **Idempotency and duplicates:** A recap names NO route and posts once, on the default one, because the
  `pns-recap` route and its Discord channel retired on 2026-09-15 and there is nothing left to fall back
  from (`pns-application/src/post_return_recap.rs`). Pinned by
  `tests/native.rs:a_recap_posts_once_on_the_default_route_even_when_the_gateway_refuses_it`, which
  answers 404 and asserts exactly one POST, to `/webhooks/pns-events`.
