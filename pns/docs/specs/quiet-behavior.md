...
- Forbidden side effects: nothing in this repo calls `pns lights pulse`. The tiers that used to are part of the
...
- Required side effects: a place NO claim names is REFUSED rather than stored, because a mute would
  ...
  `pns: lights quiet: a bare mute lasts until your quiet hours end, and `[plugins.lights]
