...
- Timeout and cancellation: no subprocess. `readable_state_file` refuses a non-regular file rather than opening
  it, because "opening a FIFO BLOCKS until the other end is opened, for READING as much as for writing"
  (`src/system.rs:readable_state_file`, and the same refusal is exercised on the sibling ring files by
