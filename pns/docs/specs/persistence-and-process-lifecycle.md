...
reminder's own card are each specified elsewhere (`presence-and-visibility.md`, `routing-and-delivery.md`,
...
link's own modification time, matching the reader. New directories use mode 0700 and new files 0600,
subject to the process mask; existing permissions and contents are preserved. No rollback, deletion or
expiry job is scheduled. Other programs may still supply this signal. See the tap contract in
`setup-and-publication.md` for path precedence, failure handling and manual undo.
...
| `presence-decisions`                    | file                    | 0600                                                        | every process that resolves the lamp map with presence armed: the event path's pulse and the tick (`src/main.rs:narrow_to_presence`), one JSON object per line (`src/presence_journal.rs:entry`) | `pns doctor`, by key rather than by position (`src/main.rs:last_narrowing`, `src/presence_journal.rs:last`)         | `presence-decisions.lock`, exclusive create, held across append, read-back, prune and publish (`src/main.rs:append_ring_line`)      | `decision_log::KEPT` = 5 lines                                                                                                                 | internal persistence detail: the only consumer is the crate's own `doctor` section                                                  |
| `presence-decisions.lock`               | file                    | 0600                                                        | the same processes (`src/main.rs:claim_ring_lock`)                                                                                                                                               | nobody reads its contents; only its mtime is judged (`src/main.rs:lock_aged_out`)                                   | `create_new`; a dead one is taken by rename to `.claim.<pid>` and removed (`src/main.rs:claim_lock`)                                | released by `HeldLock::drop`; aged out at `RING_LOCK_STALE_SECS` = 5s                                                                          | temporary process coordination: it exists only for the width of one append                                                          |
...
| `phone-attention.marker`                | file (may be a symlink) | 0600 for new files, existing modes preserved                                            | `pns tap` via `crates/pns-adapters/src/phone_marker.rs`                                                                                                                                                           | every event process's presence probe, mtime only, via `symlink_metadata` (`src/system.rs`)                          | none                                                                                                                                | none; only the mtime carries meaning                                                                                                           | public attention signal: only the marker modification time is read                                                            |
...
| `daemon-markers/`                       | directory               | umask                                                       | `src/main.rs:write_marker`, called by `clear_remind`, the reminder fire, and the `resolved` arm                                                                                                          | `src/daemon.rs:marker_exists`                                                                                       | none needed: an empty file's presence is the whole message                                                                          | one file per session that ever resolves a batch; never swept, and that accumulation is named in `src/main.rs:clear_remind`                        | internal persistence detail                                                                                                         |
| `daemon-markers/remind-<session>`          | file                    | 0600                                                        | as above (`src/remind.rs:marker_name`)                                                                                                                                                              | the daemon's `decide`                                                                                               | none                                                                                                                                | permanent until an arm removes it                                                                                                              | internal persistence detail                                                                                                         |
...
| `remind/`                                  | directory               | umask                                                       | the `blocked` hook arming a nudge (`src/main.rs:arm_remind`)                                                                                                                                        | the reminder fire (`src/main.rs:remind_mode`, `record_entries`)                                                             | a subdirectory so the fire can enumerate without pattern-matching every other state file (`src/remind.rs:remind_dir`)                     | one `<session>.pending` per outstanding approval                                                                                               | internal persistence detail                                                                                                         |
| `remind/<session>.pending`                 | file                    | 0600                                                        | `src/main.rs:arm_remind` via `publish_state_line`                                                                                                                                                   | the fire, after claiming it                                                                                         | rename to `<name>.claim.<pid>` built from the WHOLE file name, never `with_extension` (`src/remind.rs:claim_path`)                     | removed when counted, dropped, or answered                                                                                                     | internal persistence detail                                                                                                         |
| `remind/fire.lock`                         | file                    | 0600                                                        | the reminder fire (`src/main.rs:claim_fire`)                                                                                                                                                          | nobody; only its mtime is judged                                                                                    | exclusive `create_new`, explicitly NOT a rename (`src/main.rs:claim_fire`)                                                          | released by `release_fire`; aged out at `remind::FIRE_STALE_SECS` = 60s                                                                           | temporary process coordination                                                                                                      |
| `lights-blocked/`                       | directory               | umask                                                       | any event process (`src/main.rs:update_blocked_marker`, `end_blocked_wait`)                                                                                                                      | the lights tick (`src/main.rs:sweep_blocked`)                                                                       | per-file: a publish stages `<name>.new.<pid>`; the sweep takes `<name>.sweep.<pid>` by rename                                       | one file per waiting session, one epoch each; swept past the configured `lease_expiry`                                                   | internal persistence detail, with a documented operator check (`ls ~/.local/state/pns/lights-blocked`, `src/main.rs:sweep_markers`) |
| `lights-loop/`                          | directory               | umask                                                       | `pns loop begin` and `pns loop end` (typed), plus every event process from that pane renewing (`src/main.rs:renew_loop_lease`, `end_lease`)                                                      | the lights tick (`src/main.rs:sweep_leases`)                                                                        | as `lights-blocked/`; a renewal writes in place through the handle it found and creates nothing                                     | one file per pane, one epoch; swept past `lease_expiry`                                                                                  | internal persistence detail, same documented operator check                                                                         |
| `lights-shell/`                         | directory               | umask                                                       | `pns shell begin/end`, one file per calling shell pid, holding one epoch (`pns-adapters` shell marker adapter)                                                                            | the lights tick only (`src/main.rs:sweep_shell_markers`)                                                            | none: each shell is the only writer and the only ordinary remover of its own file                                                   | collected when the named pid is gone; an unreadable epoch is left while its shell lives                                                        | public external contract: shell commands publish it and the lights tick reads it                                                                    |
...
| `lights-quiet`                          | file                    | 0600                                                        | `pns lights mute` (typed) (`src/main.rs:lights_quiet`)                                                                                                                                          | the lights tick and the event path                                                                                  | none; the read-modify-write race is accepted because both racers are a human typing                                                 | one line per muted place                                                                                                                       | public external contract: the operator types it                                                                                     |
| `lights-tick.lock`                      | file                    | 0600                                                        | the lights tick (`src/main.rs:run_tick_writes`)                                                                                                                                                  | nobody; only its mtime is judged                                                                                    | exclusive `create_new` via `claim_lock`                                                                                             | released by `HeldLock::drop`; aged out at `lights_tick_stale_secs()` = `MAX_ARM_INTERVAL_SECS` + `tick_bridge_deadline(MAX_ARM_INTERVAL_SECS)` + 1 = 37s | temporary process coordination                                                                                                      |
...
- `answered/`: does not exist as a path. `answered` is a MARKER NAME used in `src/daemon.rs`'s own
  ...
  `daemon-markers/remind-<session>` (`src/remind.rs:marker_name`, prefix `remind-`).
...
| P4  | `moshi_hook_bin() <subcommand>` with the payload on stdin (`src/main.rs:spawn_moshi_hook`)                                        | the harness hook                                        | `submit_deadline()`: `[plugins.phone] ack_deadline`, else `DEFAULT_ACK_DEADLINE` = 5s (`src/main.rs:submit_deadline`)                                | `child.kill()` then `child.wait()`, and the call returns 0, which is no opinion (`src/main.rs:answer_within`) | NO, deliberately: "THE KILL REACHES THE DIRECT CHILD ONLY ... that day the kill has to widen to the process group" | the child's exit code becomes this process's exit code (`src/main.rs:moshi_decision`)                                          | `answer_within` reaps on the kill path; a child that finishes is reaped by `moshi_decision`'s `wait` |
...
| P6  | `codex exec --ephemeral --skip-git-repo-check -C <home> -s read-only -` with the prompt on stdin (`src/main.rs:summarize`)         | the `stop` hook                                         | `PNS_SUMMARIZER_DEADLINE`, else `SUMMARIZER_DEADLINE` = 30s; read cap `PROBE_READ_MAX` = 1 MiB                                                                                                            | as P5                                                                                                         | yes, as P3                                                                                                         | `None` falls back to trimming the reply (`src/main.rs:summarize`)                                                               | as P5                                                                                                |
...
- Success: `src/main.rs:an_empty_channels_dir_variable_means_the_default_not_the_current_dir` pins the
  same `resolve_path` rule on its sibling variable.
...
- Thresholds: `RING_LOCK_STALE_SECS` = 5 seconds, compared with a STRICT greater-than
  ...
  is an orphan. The siblings are `remind::FIRE_STALE_SECS` = 60 and `lights_tick_stale_secs()` = 37 at the
  production clock.
...
`src/system.rs:readable_state_file`: "EVERY READER OF THESE FILES GOES THROUGH IT ... A FIFO parks the
open forever, for READING as much as for writing, which wedges the hook that appended or the command a
human is waiting on. A file some other hand grew to gigabytes is otherwise learned about by allocating
it." The same refusal appears at the append (`append_ring_line`), at the spool (`src/daemon.rs:peek`,
...
- Compatibility contract: `readable_state_file` returns `io::Error`s rather than an absence "so a caller
  that has to tell 'there is no file' from 'the file could not be read' still can: the doctor says a
...
adoption scan matches), and read. The hold remains through the replay dispatch. `ReturnMoment::complete`
removes only this run's holds after the attempt returns, including a failed attempt. An unwind or process
exit before completion leaves the hold for later adoption.
...
`crates/pns-adapters/src/protocols/journal_claims/take.rs:take_claim` preserves a failed read on disk.
`crates/pns-adapters/src/protocols/return_window/mod.rs:FileReturnMoment` owns successful holds until the
application calls `complete`; it deliberately has no destructor that consumes them.
...
- Timeout and cancellation: an attempt that returns is completed, even on delivery failure. An unwind or
  process death before that return leaves an adoptable hold. No retry policy is added here.
...
the reply and the summarizer, because "Stop is asynchronous, so the next prompt can arrive while this one
is still summarizing."
...
- Success: `tests/hooks.rs:stopping_consumes_the_marker_so_a_second_stop_cannot_re_fire_the_tier`;
  ...
  `tests/hooks.rs:a_prompt_arriving_while_the_previous_stop_summarizes_keeps_its_own_marker`;
...
- Required side effects: the marker is created exclusively with mode `0600`, so a later prompt keeps the
  first marker's bytes and a dangling symlink never becomes an open of its target.
...
- Compatibility contract: the name and one-epoch contents stay unchanged. The approved retention repair
  ...
  Exactly 604800 seconds, future epochs, and an unavailable current clock do not expire a marker. The
  tick takes an exclusive sweep claim, rereads its owned file, and removes only an expired claim. A fresh
  or unreadable claim is restored by an exclusive hard link; a later prompt is never replaced. A failed
  restoration retains the claim. Links, directories, unsafe names and claims another run already owns are
  left alone. This does not add collection of answered or stranded Stop claims.
...
- Timeout and cancellation: the lease's own `lease_expiry` is what expires it.
...
- Privacy: "NO FREE TEXT REACHES THE SPOOL. `args` are visible in the spool file and in whatever the
  daemon logs, and the detail is the operator's own question, so it lives in the record and `pns remind`
  takes no argument" (`src/main.rs:arm_remind`).
...
- Thresholds: `CHILD_TICKS` = 30 ticks (30 seconds at the production clock). For the `lights` job the
  bound is `max(tick*30, MAX_ARM_INTERVAL_SECS + tick_bridge_deadline(MAX_ARM_INTERVAL_SECS) + tick)` =
...
- Success: `tests/hooks.rs:a_submission_that_dies_without_answering_is_not_a_decision`;
  `tests/hooks.rs:a_moshi_that_never_answers_stops_holding_the_operators_prompt`;
...
- Thresholds: `[plugins.phone] ack_deadline`, else `DEFAULT_ACK_DEADLINE` = 5 seconds. The poll
...
- Compatibility contract: the child's exit code is passed through untouched, because a harness that
  reads the hook's exit code is entitled to whatever moshi said.
...
### 26. The reminder fire owns the whole window by exclusive create

Given several remind jobs waking at once over one `remind/` directory

When each tries to fire

Then exactly one creates `remind/fire.lock` with an exclusive `create_new` and owns the whole window; every
...
- Fail direction: fail-closed toward a second card, and LOUD about a claim it could not give back:
  `src/main.rs:release_fire` prints "pns remind: the fire claim {} could not be given up ({error}); the next
  fire waits it out".
- Thresholds: `remind::FIRE_STALE_SECS` = 60 seconds, strict greater-than as in behavior 5. "A minute is a
...
- Forbidden side effects: the arm clears this session's previous marker BEFORE it publishes the new
  ...
  approval's marker still on disk and drops it as answered" (`src/main.rs:arm_remind`). The arm performs "NO
...
- Timeout and cancellation: the whole feature is off when `[remind] delay` is absent or unreadable
  (`REMIND_OFF` = 0), and a config that turned it off between arming and firing DROPS the records.
- Idempotency and duplicates: one card per window whatever the count. The claim suffix is built from the
  ...
  nudge and the other can be delivered twice" (`src/remind.rs:claim_path`).
...
- Compatibility contract: the record suffix is `.pending` and IS the whole enumeration test, "which is
  ...
  so it can never be read back as a record and taken a second time" (`src/remind.rs:session_of`).
...
## Transactional internal repositories

Behaviors 28 through 36 specify the SQLite adapter and its explicit import operation. Behaviors 42
through 45 add consumer cutover and diagnostics. Coordination, markers, spool, configuration and
externally observed state remain files. Delivery-ledger storage is specified separately in behaviors 37
through 41 below.

### 28. A private versioned database preserves committed state

Given a configured state directory, when a repository first opens, then it creates `pns.db` and its
sidecars as private regular files, enables write-ahead logging and full synchronization, and commits the
versioned schema explicitly. Reopening retains committed records. A future version is refused before
changing its journal mode or data. A symlink, directory or publicly readable database is refused without
following, replacing or repairing it. Ordinary storage errors are returned, never converted into an empty
in-memory database.

### 29. Repository retention and reads keep their existing codecs

Given typed decisions, journal events, activity, presence decisions or policy-setting receipts, when
records are appended, then arrival order and the existing codecs are preserved. Retention keeps five
decisions, 25 journal entries, 150 activity entries, five presence decisions and 20 audit receipts.
Exactly that many remain intact; the next append removes only the oldest pending entry. Decision history
omits event free text. Activity reads exclude the near edge, include the far edge and exclude unknown
clocks; full unsigned epoch values remain representable. The same decision, journal and activity
contracts run against a concrete in-memory repository and SQLite.
...
Given a near edge and pending journal, when a return claims both, then one transaction advances the edge
monotonically and reserves the pending rows. An edge-only request leaves the journal pending; an unknown
clock does not invent an edge. A busy claim is refused without consuming anything. A live owner cannot be
displaced, and pending retention cannot prune its held batch. Drop or process exit leaves an unfinished
...
Given quiet expiry, a staleness episode, two complaint memories, held lamps, mutes, news or a working
streak, when the corresponding repository changes, then only that record family changes. Reopening
retains its bytes or typed values; clearing one complaint cannot clear the other. Held phases and mute
order retain their codec bounds. Replacing a held or muted set commits atomically. News merges both kinds
without moving either epoch backwards; irrelevant events and unknown clocks do not initialize storage. A
streak survives the exact 120-second grace and resets one second later.

### 33. First-run import preserves readable legacy records and is idempotent

Given legacy internal records and no active legacy owner, when `import_legacy` runs, then records and
per-family completion commit together. Readable history retains its original line endings and missing
final newline. An empty ring stays distinct from an absent ring. Appending imported history adds the same
missing separator and prunes at the same boundary as the existing file writer. Original files remain
byte-for-byte untouched. Once completion is committed, another import cannot replay old files over newer
records or duplicate history. A failed or interrupted transaction commits neither rows nor completion
markers, and the next invocation can retry.

### 34. Failed imports remain distinguishable from absence

Given an unreadable or malformed legacy record, when import runs, then the other readable families can
commit, while the failure's family and non-secret reason remain available through `import_failures`. A
bounded daemon-log entry names that family where possible. Unknown history, quiet expiry and lamp records
retain their existing reader failure directions. A torn return edge stays unknown rather than becoming
epoch zero. Successful semantic replacement clears only its own failed-import status; the original
unreadable bytes remain available for recovery.

### 35. Import respects legacy ownership and preserves abandoned batches

Given a live legacy journal or return-window owner, or a ring lock at or inside its five-second stale
edge, when import is attempted, then it refuses before recording any family complete. Unknown or future
clocks do not age out a ring lock. An abandoned journal batch transfers in the existing
modification-time/path order and stays outside pending retention. Abandoned return epochs merge
monotonically. Unreadable abandoned records are reported and retained. Import never removes a legacy
ownership file. These checks supplement the required rollout quiescence; they cannot prevent an old
binary from starting after the snapshot.

### 36. Application repository ports preserve unknown reads and best-effort results

Given the application-owned lamp, complaint, presence and staleness ports, when SQLite implements them,
then unreadable held lamps remain unknown, mute failures retain one diagnostic, and failed publications
cannot become success. The mute diagnostic names replacement of the stored record. A failed streak
publication still returns the computed next streak through one bounded read-only fallback, without
claiming it was stored. Complaint kinds and staleness remain independent. Presence records contain the
caller's actual snapshot and narrowing; house reads return committed news. The application, not these
repositories, owns physical lamp changes and notification ordering.

## Delivery ledger storage

These contracts specify the application-owned `DeliveryLedger` capability and its SQLite implementation.
The submission, destination and daemon use cases own dispatch, transport acknowledgement and retry
timing. Decision 0013 defines the at-least-once policy.

### 37. A logical submission gets one committed plan and one dispatch owner

Given an original producer and request id, when preparation succeeds, then one transaction stores that
identity, every rendered event field and each destination instance's resolved route, reporting mode and
decorative flag before returning dispatch claims. Different identities retain separate monotonically
ordered events even when their bodies match. An identical duplicate returns existing history without
claims, including while the first writer is active. Conflicting event fields, routes or leg order are
refused without overwriting anything. Duplicate destination instances are invalid; an empty plan is a
valid retained submission. A failed leg insert rolls back the event and every leg and attempt together.

### 38. Each attempt retains an honest outcome

Given a planned leg, when it is first leased, then its generation is recorded as unknown with the lease
expiry as its retry instant. That history states an unresolved outcome, not a proven failure. Only an
explicit transport acknowledgement can settle it as acknowledged; printable silence cannot. Failed,
unlaunched and unknown completions retain their exact details and next retry instant. Acknowledged legs
remain inspectable and are never offered for retry. Re-leasing appends a generation without erasing prior
attempts. Event sequence and leg position give the retry scan its best-effort order.

### 39. Lease ownership prevents duplicate active work and stale completion

Given an active initial or retry claimant, when another process asks for retry work, then that claimant's
leg is excluded until its lease expires or its process is known to be gone. At one instant before expiry
it remains owned; at expiry and afterward it is eligible. A completed unconfirmed attempt is eligible at
its retry instant, not one instant earlier. Full unsigned epochs are preserved. A non-positive lease
window is refused before mutation. A retry retains the original identity, event and resolved leg facts.
Completion requires the leg's current generation and opaque token, including database ownership; a
foreign, stale or already settled claim is refused. An uncontested late completion may still settle its
own generation. Killing an owned claimant preserves its unknown attempt and lets a later claimant retry.

### 40. Storage failures do not invent delivery or consume ownership

Given a busy or failed database, when a ledger operation cannot commit, then it returns a typed storage
failure and attempts a non-secret daemon diagnostic. The busy wait uses the existing bounded timeout. The
application decides how to continue delivery; no storage method sends anything. If an outcome write
...
tables while retaining existing record tables and bytes. A failed ledger migration leaves no partial
tables and retains version 1. The same transaction applies later supported migrations before publishing
the current version. A database newer than the supported version is refused before changing its journal
mode or records. The private-file, full-synchronization and legacy-import contracts continue to apply.
Opening storage never restarts a daemon.

## Internal-record consumer cutover

### 42. The first consumer access imports before exposing records

Given existing internal files, when a `for_records` consumer first reads or writes, then the legacy
import commits before the operation can succeed. Construction alone writes nothing. An active legacy
owner or failed transaction refuses both access directions; neither returns empty success or writes a
replacement record. Completed import checks do not acquire a writer transaction, and later file changes
cannot supersede database state. A failed family remains unknown and diagnostic while other families
remain usable. Original files stay intact. This requires the rollout quiescence in decision 0014.

### 43. Per-leg outcomes revise one retained logical decision

Given an original producer/request pair, when its initial decision is recorded, then that pair occupies
one position in the existing five-entry ring. Repeating it preserves the recorded outcome; a different
producer or request remains distinct. The initial `legs=none` states that no outcome has been recorded.
When a leg completes, revision appends its registered name and printable verdict, or replaces that name's
verdict on a later attempt. The codec preserves the prefix, every other leg and their encounter order; no
retry reconstructs the original policy or hook facts. Reading and updating occur in one transaction, so
concurrent completions both survive. Malformed or duplicate suffix entries and delimiter-bearing names
are refused without writing. A revision for an already-pruned identity returns false without inserting a
replacement. Imported null-identity rows preserve their exact newline and pruning behavior. A refused
revision returns an error, reports a bounded diagnostic and leaves the prior outcome intact. Schema
version 3 adds identity columns and their unique index in the existing migration transaction; it does not
replace the ledger or its acknowledgement policy.

### 44. Doctor reports retained import failures after history

Given retained import failures, when doctor finishes its history sections, then it prints each family and
non-secret reason and states that the legacy file is retained. If the import check itself is unavailable,
it names that failure instead of claiming a healthy store. Both cases leave the delivery-health exit code
unchanged and disclose no imported record content. A first read may initialize and import storage; doctor
still appends no event records and claims no return batch.

### 45. Direct record misses remain best effort and diagnostic

Given a failed database, when an event attempts to record lamp news, a return edge or a policy-setting
receipt, then each method returns its failure and attempts a bounded daemon-log diagnostic naming its
