...
| The summarizer's access to live Codex credentials | `~/.config/pns/codex-home`, a stripped Codex home with the live auth SYMLINKED                   | Three summarizer children running `codex exec --ephemeral --skip-git-repo-check -s read-only` in it: the Stop hook's turn summary, a recap's `codex` summarizer and the doctor's summarizer row                          | Any other reader: the directory is created with mode `0700` (`main::summarizer_home`) and the config inside it with `0o600`                                                                                                                                              | `NOT ESTABLISHED:` no test found asserting the mode of the summarizer home or its config file. The modes are set at `src/main.rs:summarizer_home` and `src/main.rs:2306`; nothing in `tests/` reads them back                                                                                                                                                                                                                                                                                                                                  |
