...
| `main::rendered_plainly`            | `hooks::flattened`, then every character for which `recap::is_invisible` answers true is DROPPED                                                                                                                                                 | Everything `flattened` leaves                                                                  | `main::model_switch_detail` on `from_model` and `to_model`                                                       | Every other rendered field. Stated at `main::rendered_plainly`: widening `flattened` itself "would let every other field silently start allowing format characters through too". The one caller earns it because it compares two names for equality                                                                                 | `tests/hooks.rs:an_auto_switch_strips_a_unicode_format_character_from_the_name`                                                                     |
...
| Decision ring and journal read-back (`main::RING_READ_MAX`)            | 256 KiB                                                                                                             | Read and pruned                               | `readable_state_file` returns `FileTooLarge`. `append_ring_line`'s heal fires and the file is republished holding ONLY the line just written                                                                              |
...
- Required side effects: none.
- Forbidden side effects: no widening of `hooks::flattened` itself. The doc comment states why: one
  caller earns the strip, and moving it inside `flattened` "would let every other field silently start
...
- Success: `readable_state_file` stats with `symlink_metadata`, refuses anything that is not a regular file
